...
public:
   ...
   *
   * For \link WServer::addResource() static resources\endlink the internal
   * path is empty.
   */
   ...
   *
   * For static resources, this is the path that the
   * resource was registered at using WServer::addResource(),
   * including the deployment path. If the same resource is
   * registered at multiple paths, url() will reflect the last path.
   */
