...
public:
  ...
  void setFormModel(std::shared_ptr<FormModel<C>> model)
  {
    ...
    ViewAction action(model_->session(), model_.get(), fieldData_);
  ...
  void setFormDelegate(Wt::WFormModel::Field field, std::shared_ptr<Wt::Form::WAbstractFormDelegate> delegate)
  {
    ...
    if (!delegate) {
      auto it = fieldData_.find(field);
      if (it != fieldData_.end() && !it->second.validator) {
        fieldData_.erase(it);
      }
    } else {
      fieldData_[field].formDelegate = std::move(delegate);
    }
  }

  /*! \brief Sets the validator of a field
   *
   * This method will set the validator for the given \p field,
   * overriding the validator generated by the form delegate, unless
   * \p newValidator is a \p nullptr, in which case the validator will
   * be set to the validator generated by the form delegate.
   *
   * \note Unless \p newValidator is nullptr, customizeValidator() will
   *       not be called.
   */
  void setValidator(Wt::WFormModel::Field field, std::shared_ptr<Wt::WValidator> newValidator)
  {
    if (!newValidator) {
      auto it = fieldData_.find(field);
      if (it != fieldData_.end()) {
        if (it->second.formDelegate) {
          // reset to formDelegate validator
          it->second.validator = nullptr;
          newValidator = std::move(validator(field));
        } else {
          fieldData_.erase(it);
        }
      }
    } else {
      fieldData_[field].validator = newValidator;
    }

    if (model_) {
      model_->setValidator(field, std::move(newValidator));
      updateView(model_.get());
    }
  }
...
protected:
   ...
   *
   * \note This function is only applied to validators generated
   *       automatically (by the delegate) It is not applied to
   *       validators set using setValidator().
   */
...
private:
  ...
  std::map<std::string, ViewAction::FieldData> fieldData_;
  ...
  {
    auto data = fieldData_.find(field);
    if (data != fieldData_.end()) {
      if (data->second.validator) {
        return data->second.validator;
      }

      if (data->second.formDelegate) {
        std::shared_ptr<Wt::WValidator> validator = data->second.formDelegate->createValidator();
