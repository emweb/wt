...
window._$_APP_CLASS_$_=new (function(){function M(c){c=p.pageCoordinates(c);G=c.x;H=c.y}function N(){var c=_$_WT_CLASS_$_.history.getCurrentState();if(!(c.length>0&&c.substr(0,1)!="/"))if(m!=c){m=c;setTimeout(function(){a(null,"hash",null,true)},1)}}function F(c){if(!(m==c||!m&&c=="/")){m=c;p.history.navigate(c,false)}}function K(){document.body.ondragstart=function(){return false}}function P(c,d){var f=p.target(d);if(f)if(f.offsetWidth>f.clientWidth||f.offsetHeight>f.clientHeight){var h=p.widgetPageCoordinates(f),
