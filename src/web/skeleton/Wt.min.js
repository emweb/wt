...
return c}function Ra(){for(var c,d="",f=false,l=0;l<J.length;++l){c=l>0?"&e"+l:"&";f=f||J[l].feedback;d+=c+J[l].data.join(c);if(J[l].evAckId<Z)d+=c+"evAckId="+J[l].evAckId}Ma=Ma.concat(J);J=[];return{feedback:f,result:d}}function Ca(c){da=true;Na=c;if(Ia){clearInterval(Ia);Ia=null}if(ia){clearTimeout(ia);ia=null}if(M){clearTimeout(M);M=null}qa.cancel();c=$("#Wt-timers");c.length>0&&q.setHtml(c.get(0),"",false)}function Sa(){q.history._initTimeout();Ja==0&&xa(null,"keepAlive",null,false)}function La(c){if(!q.isIEMobile)document.title=
