...
return y}}}()}),_$_APP_CLASS_$_=new (function(){function x(d){d=g.pageCoordinates(d);o=d.x;p=d.y}function t(){var d=g.history.getCurrentState();if(s!=d){s=d;setTimeout(function(){A(null,"hash",null,true)},1)}}function C(d){if(s!=d){s=d;g.history.navigate(escape(d))}}function Y(){window.onresize=function(){q()};document.body.ondragstart=function(){return false}}function k(d,c){var e=J;e.object=g.getElement(d.getAttribute("dwid"));if(e.object==null)return true;e.sourceId=d.getAttribute("dsid");e.objectPrevStyle=
{position:e.object.style.position,display:e.object.style.display,left:e.object.style.left,top:e.object.style.top,className:e.object.className};e.object.parentNode.removeChild(e.object);e.object.style.position="absolute";e.object.className="";document.body.appendChild(e.object);g.capture(e.object);e.object.onmousemove=U;e.object.onmouseup=D;e.offsetX=-4;e.offsetY=-4;e.dropTarget=null;e.mimeType=d.getAttribute("dmt");e.xy=g.pageCoordinates(c);g.cancelEvent(c);return false}function U(d){if(J.object!=
null){var c=J,e=g.pageCoordinates(d);if(c.object.style.display!=""&&c.xy.x!=e.x&&c.xy.y!=e.y)c.object.style.display="";c.object.style.left=e.x-c.offsetX+"px";c.object.style.top=e.y-c.offsetY+"px";e=c.dropTarget;var h=d.target||d.srcElement,i="{"+c.mimeType+":",j=null;for(c.dropTarget=null;h;){j=h.getAttribute("amts");if(j!=null&&j.indexOf(i)!=-1){c.dropTarget=h;break}h=h.parentNode;if(!h.tagName||g.hasTag(h,"HTML"))break}if(c.dropTarget!=e){if(c.dropTarget){h=j.indexOf(i)+i.length;var r=j.indexOf("}",
h);j=j.substring(h,r);if(j.length!=0){c.dropTarget.setAttribute("dos",c.dropTarget.className);c.dropTarget.className=c.dropTarget.className+" "+j}}else c.object.styleClass="";if(e!=null){e.handleDragDrop&&e.handleDragDrop("end",c.object,d,"",i);j=e.getAttribute("dos");if(j!=null)e.className=j}}if(c.dropTarget)if(c.dropTarget.handleDragDrop)c.dropTarget.handleDragDrop("drag",c.object,d,"",i);else c.object.className="Wt-valid-drop";g.cancelEvent(d);return false}return true}function D(d){g.capture(null);
var c=J;if(c.object){if(c.dropTarget){var e=c.dropTarget.getAttribute("dos");if(e!=null)c.dropTarget.className=e;c.dropTarget.handleDragDrop?c.dropTarget.handleDragDrop("drop",c.object,d,c.sourceId,c.mimeType):O(c.dropTarget,{name:"_drop",eventObject:c.dropTarget,event:d},c.sourceId,c.mimeType)}c.object.style.position=c.objectPrevStyle.position;c.object.style.display=c.objectPrevStyle.display;c.object.style.left=c.objectPrevStyle.left;c.object.style.top=c.objectPrevStyle.top;c.object.className=c.objectPrevStyle.className;
c.object=null}}function R(d,c){var e,h;h=d.event;c=c>0?"&e"+c:"&";e=c+"signal="+d.signal;if(d.id){e+=c+"id="+d.id+c+"name="+encodeURIComponent(d.name)+c+"an="+d.args.length;for(var i=0;i<d.args.length;++i)e+=c+"a"+i+"="+encodeURIComponent(d.args[i])}for(var j=0;j<B.length;++j){var r=g.getElement(B[j]),v=null,W;if(r!=null){if(r.type=="select-multiple"){i=0;for(W=r.options.length;i<W;i++)if(r.options[i].selected)e+=c+B[j]+"="+encodeURIComponent(r.options[i].value)}else if(g.hasTag(r,"SPAN")){i=0;for(W=
r.childNodes.length;i<W;++i)if(r.childNodes[i].type=="checkbox"){i=r.childNodes[i];if(i.style.display=="none")v="indeterminate";else if(i.checked)v=i.value;break}}else if(r.type=="checkbox"||r.type=="radio")if(r.indeterminate)v="indeterminate";else{if(r.checked)v=r.value}else if(r.type!="file")v=$(r).hasClass("Wt-edit-emptyText")?"":""+r.value;if(v!=null)e+=c+B[j]+"="+encodeURIComponent(v)}}if(s!=null)e+=c+"_="+encodeURIComponent(unescape(s));if(!h){d.data=e;return d}for(j=h.target||h.srcElement;!j.id&&
j.parentNode;)j=j.parentNode;if(j.id)e+=c+"tid="+j.id;try{e+=c+"type="+h.type}catch(ja){}if(h.clientX||h.clientY)e+=c+"clientX="+h.clientX+c+"clientY="+h.clientY;v=g.pageCoordinates(h);j=v.x;v=v.y;if(j||v){e+=c+"documentX="+j+c+"documentY="+v;e+=c+"dragdX="+(j-o)+c+"dragdY="+(v-p)}if(h.screenX||h.screenY)e+=c+"screenX="+h.screenX+c+"screenY="+h.screenY;if(d.object&&d.object.nodeType!=9){i=g.widgetPageCoordinates(d.object);e+=c+"scrollX="+d.object.scrollLeft+c+"scrollY="+d.object.scrollTop+c+"width="+
d.object.clientWidth+c+"height="+d.object.clientHeight+c+"widgetX="+(j-i.x)+c+"widgetY="+(v-i.y)}if(h.which)e+=c+"right="+(h.which==3);else if(h.button)e+=c+"right="+(h.button==2);if(typeof h.keyCode!=="undefined")e+=c+"keyCode="+h.keyCode;if(typeof h.charCode!=="undefined")e+=c+"charCode="+h.charCode;if(h.altKey)e+=c+"altKey=1";if(h.ctrlKey)e+=c+"ctrlKey=1";if(h.metaKey)e+=c+"metaKey=1";if(h.shiftKey)e+=c+"shiftKey=1";d.data=e;return d}function a(){for(var d="",c=false,e=0;e<z.length;++e){c=c||z[e].feedback;
d+=z[e].data}Z=z;z=[];return{feedback:c,result:d}}function b(){g.history._initTimeout();T==0&&A(null,"none",null,false);aa=setTimeout(b,_$_KEEP_ALIVE_$_000)}function f(d){if(!g.isIEMobile)document.title=d}function m(){g.history._initialize();Y();if(!da){da=true;_$_ONLOAD_$_();aa=setTimeout(b,_$_KEEP_ALIVE_$_000)}}function l(d){clearTimeout(d);document.body.style.cursor="auto";if(X!=null){try{X()}catch(c){}X=null}}function n(){document.body.style.cursor="wait";X=hideLoadingIndicator;showLoadingIndicator()}
function E(d){ea=d}function q(d){if(d)if(window.execScript)window.execScript(d);else with(window)eval(d);_$_APP_CLASS_$_._p_.autoJavaScript()}function u(d,c,e){if(!ba){if(d==0){_$_$ifnot_DEBUG_$_();try{_$_$endif_$_();q(c);_$_$ifnot_DEBUG_$_()}catch(h){alert("Wt internal error: "+h+", code: "+h.code+", description: "+h.description)}_$_$endif_$_();e&&l(e)}else z=Z.concat(z);Z=[];if(M){clearTimeout(M);M=null}G=null;if(d>0)++T;else T=0;if(ea||z.length>0)if(d==1){d=Math.min(12E4,Math.exp(T)*500);P=setTimeout(function(){w()},
