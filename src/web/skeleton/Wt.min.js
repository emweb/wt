...
f=b.duplicate();f.moveToElementText(a);var h=0;if(b.text.length>1){h-=b.text.length;if(h<0)h=0}a=-1+h;for(f.moveStart("character",h);f.inRange(b);){f.moveStart("character");a++}b=b.text.replace(/\r/g,"");return{start:a,end:b.length+a}}else{f=$(a).val();a=document.selection.createRange().duplicate();a.moveEnd("character",f.length);b=a.text==""?f.length:f.lastIndexOf(a.text);a=document.selection.createRange().duplicate();a.moveStart("character",-f.length);return{start:b,end:a.text.length}}else return a.selectionStart||
...
null;J=a();if(N!==q){q=N;f(q);J=q?q:G;n=location.hash=J;b()}else if(J!==n){n=J;h(J)}},50);I=true;x!=null&&x()}}function o(){if(!r){var l=a(),p=history.length;U&&clearInterval(U);U=setInterval(function(){var q,n;q=a();n=history.length;if(q!==l){l=q;p=n;f(l);b()}else if(n!==p&&u){l=q;p=n;q=M[p-1];f(q);b()}},50)}}function E(){var l;l=v.value.split("|");if(l.length>1){G=l[0];C=l[1]}if(l.length>2)M=l[2].split(",");if(r)k();else{o();I=true;x!=null&&x()}}var u=false,r=false,y=false,x=null,F=null,v=null,
...
n.history.getCurrentState();if(O!=d){O=d;setTimeout(function(){y(null,"hash",null,true)},1)}}function D(d){if(O!=d){O=d;n.history.navigate(escape(d))}}function T(){window.onresize=function(){E()};document.body.ondragstart=function(){return false}}function ca(d,c){var e=aa;e.object=n.getElement(d.getAttribute("dwid"));if(e.object==null)return true;e.sourceId=d.getAttribute("dsid");e.objectPrevStyle={position:e.object.style.position,display:e.object.style.display,left:e.object.style.left,top:e.object.style.top,
className:e.object.className};e.object.parentNode.removeChild(e.object);e.object.style.position="absolute";e.object.className="";document.body.appendChild(e.object);n.capture(null);n.capture(e.object);e.object.onmousemove=g;e.object.onmouseup=Z;e.offsetX=-4;e.offsetY=-4;e.dropTarget=null;e.mimeType=d.getAttribute("dmt");e.xy=n.pageCoordinates(c);n.cancelEvent(c);return false}function g(d){if(aa.object!=null){var c=aa,e=n.pageCoordinates(d);if(c.object.style.display!=""&&c.xy.x!=e.x&&c.xy.y!=e.y)c.object.style.display=
"";c.object.style.left=e.x-c.offsetX+"px";c.object.style.top=e.y-c.offsetY+"px";e=c.dropTarget;var j=d.target||d.srcElement,m="{"+c.mimeType+":",i=null;for(c.dropTarget=null;j;){i=j.getAttribute("amts");if(i!=null&&i.indexOf(m)!=-1){c.dropTarget=j;break}j=j.parentNode;if(!j.tagName||n.hasTag(j,"HTML"))break}if(c.dropTarget!=e){if(c.dropTarget){j=i.indexOf(m)+m.length;var s=i.indexOf("}",j);i=i.substring(j,s);if(i.length!=0){c.dropTarget.setAttribute("dos",c.dropTarget.className);c.dropTarget.className=
c.dropTarget.className+" "+i}}else c.object.styleClass="";if(e!=null){e.handleDragDrop&&e.handleDragDrop("end",c.object,d,"",m);i=e.getAttribute("dos");if(i!=null)e.className=i}}if(c.dropTarget)if(c.dropTarget.handleDragDrop)c.dropTarget.handleDragDrop("drag",c.object,d,"",m);else c.object.className="Wt-valid-drop";n.cancelEvent(d);return false}return true}function Z(d){n.capture(null);var c=aa;if(c.object){if(c.dropTarget){var e=c.dropTarget.getAttribute("dos");if(e!=null)c.dropTarget.className=
...
if(d.id){e+=c+"id="+d.id+c+"name="+encodeURIComponent(d.name)+c+"an="+d.args.length;for(var m=0;m<d.args.length;++m)e+=c+"a"+m+"="+encodeURIComponent(d.args[m])}for(var i=0;i<V.length;++i){var s=n.getElement(V[i]),z=null,fa;if(s!=null){if(s.type=="select-multiple"){m=0;for(fa=s.options.length;m<fa;m++)if(s.options[m].selected)e+=c+V[i]+"="+encodeURIComponent(s.options[m].value)}else if(s.type=="checkbox"||s.type=="radio")if(s.indeterminate||s.style.opacity=="0.5")z="i";else{if(s.checked)z=s.value}else if(s.type!=
"file"){z=$(s).hasClass("Wt-edit-emptyText")?"":""+s.value;if(n.hasFocus(s)){m=n.getSelectionRange(s);e+=c+"selstart="+m.start+c+"selend="+m.end}}if(z!=null)e+=c+V[i]+"="+encodeURIComponent(z)}}if(document.activeElement)e+=c+"focus="+document.activeElement.id;if(O!=null)e+=c+"_="+encodeURIComponent(unescape(O));if(!j){d.data=e;return d}for(i=j.target||j.srcElement;!i.id&&i.parentNode;)i=i.parentNode;if(i.id)e+=c+"tid="+i.id;try{e+=c+"type="+j.type}catch(ma){}if(j.clientX||j.clientY)e+=c+"clientX="+
j.clientX+c+"clientY="+j.clientY;z=n.pageCoordinates(j);i=z.x;z=z.y;if(i||z){e+=c+"documentX="+i+c+"documentY="+z;e+=c+"dragdX="+(i-N)+c+"dragdY="+(z-J);m=0;if(j.wheelDelta)m=j.wheelDelta>0?1:-1;else if(j.detail)m=j.detail<0?1:-1;e+=c+"wheel="+m}if(j.screenX||j.screenY)e+=c+"screenX="+j.screenX+c+"screenY="+j.screenY;if(d.object&&d.object.nodeType!=9){m=n.widgetPageCoordinates(d.object);e+=c+"scrollX="+d.object.scrollLeft+c+"scrollY="+d.object.scrollTop+c+"width="+d.object.clientWidth+c+"height="+
d.object.clientHeight+c+"widgetX="+(i-m.x)+c+"widgetY="+(z-m.y)}i=n.button(j);if(!i)if(n.buttons&1)i=1;else if(n.buttons&2)i=2;else if(n.buttons&4)i=4;e+=c+"button="+i;if(typeof j.keyCode!=="undefined")e+=c+"keyCode="+j.keyCode;if(typeof j.charCode!=="undefined")e+=c+"charCode="+j.charCode;if(j.altKey)e+=c+"altKey=1";if(j.ctrlKey)e+=c+"ctrlKey=1";if(j.metaKey)e+=c+"metaKey=1";if(j.shiftKey)e+=c+"shiftKey=1";d.data=e;return d}function S(){for(var d="",c=false,e=0;e<A.length;++e){c=c||A[e].feedback;
...
setCloseMessage:p};this.WT=_$_WT_CLASS_$_;this.emit=I}),WtSignalEmit=_$_APP_CLASS_$_.emit;window.WtScriptLoaded=false;function onLoad(){if(window.WtScriptLoaded){_$_WT_CLASS_$_.history.initialize("Wt-history-field","Wt-history-iframe");_$_APP_CLASS_$_._p_.load()}else window.isLoaded=true};
