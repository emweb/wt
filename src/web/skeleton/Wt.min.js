...
j},_initialize:function(){I!=null&&x()},_initTimeout:function(){m()},register:function(j,o){if(!B){y=D=escape(j);J=o}},initialize:function(j,o){if(!B){var p=navigator.vendor||"";if(p!=="KDE")if(typeof window.opera!=="undefined")A=true;else if(typeof document.all!=="undefined")t=true;else if(p.indexOf("Apple Computer, Inc.")>-1)r=true;if(typeof j==="string")j=document.getElementById(j);if(!(!j||j.tagName.toUpperCase()!=="TEXTAREA"&&(j.tagName.toUpperCase()!=="INPUT"||j.type!=="hidden"&&j.type!=="text"))){I=
...
g.getAttribute("amts");if(i!=null&&i.indexOf(h)!=-1){d.dropTarget=g;break}g=g.parentNode;if(!g.tagName||q.hasTag(g,"HTML"))break}if(d.dropTarget!=e){if(d.dropTarget){g=i.indexOf(h)+h.length;c=i.indexOf("}",g);i=i.substring(g,c);if(i.length!=0){d.dropTarget.setAttribute("dos",d.dropTarget.className);d.dropTarget.className=d.dropTarget.className+" "+i}}else d.object.styleClass="";if(e!=null){e.handleDragDrop&&e.handleDragDrop("end",d.object,c,"",h);i=e.getAttribute("dos");if(i!=null)e.className=i}}if(d.dropTarget)if(d.dropTarget.handleDragDrop)d.dropTarget.handleDragDrop("drag",
d.object,c,"",h);else d.object.className="Wt-valid-drop";q.cancelEvent(c);return false}return true}function x(c){if(!c)c=window.event;if(H!=null){var d=H;f(null);d.onmouseup&&d.onmouseup(c);return false}d=T;if(d.object){if(d.dropTarget){var e=d.dropTarget.getAttribute("dos");if(e!=null)d.dropTarget.className=e;d.dropTarget.handleDragDrop?d.dropTarget.handleDragDrop("drop",d.object,c,d.sourceId,d.mimeType):w(d.dropTarget,{name:"_drop",eventObject:d.dropTarget,event:c},d.sourceId,d.mimeType)}d.object.style.position=
...
function I(c){clearTimeout(c);document.body.style.cursor="auto";if(U!=null){try{U()}catch(d){}U=null}}function B(){document.body.style.cursor="wait";U=hideLoadingIndicator;showLoadingIndicator()}function Q(c){ca=c}function E(c){with(window){c&&eval(c);_$_APP_CLASS_$_._p_.autoJavaScript()}}function D(c,d,e){if(!X){if(c==0){_$_$ifnot_DEBUG_$_();try{_$_$endif_$_();E(d);_$_$ifnot_DEBUG_$_()}catch(g){alert("Wt internal error: "+g+", code: "+g.code+", description: "+g.description)}_$_$endif_$_();e&&I(e)}else v=
V.concat(v);V=[];if(K){clearTimeout(K);K=null}C=null;if(c>0)++S;else S=0;if(ca||v.length>0)if(c==1){c=Math.min(12E4,Math.exp(S)*500);M=setTimeout(function(){p()},c)}else p()}}function y(){C.abort();K=C=null;p()}function J(c,d,e,g){H&&c==H&&e.type=="mouseup"&&f(null);_$_$if_STRICTLY_SERIALIZED_EVENTS_$_();if(!C){_$_$endif_$_();var h={},i=v.length;h.object=c;h.signal=d;h.event=e;h.feedback=g;v[i]=r(h,i);j();E();_$_$if_STRICTLY_SERIALIZED_EVENTS_$_()}_$_$endif_$_()}function j(){if(C!=null&&K!=null){clearTimeout(K);
...
0&&c.callback(c.images)};q.history.register(_$_INITIAL_HASH_$_,a);this._p_={loadScript:ia,onJsLoad:G,setTitle:z,update:J,quit:function(){X=true;clearTimeout(W)},setFormObjects:function(c){R=c},saveDownPos:l,addTimerEvent:P,load:F,handleResponse:D,setServerPush:Q,dragStart:k,dragDrag:m,dragEnd:x,capture:f,onHashChange:a,setHash:b,ImagePreloader:O,autoJavaScript:function(){with(window)_$_AUTO_JAVASCRIPT_$_()},response:o};this.emit=w}),WtSignalEmit=_$_APP_CLASS_$_.emit;window.WtScriptLoaded=false;
