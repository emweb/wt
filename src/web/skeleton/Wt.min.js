...
if(a.offsetParent){var t=g.widgetPageCoordinates(a.offsetParent),q=["left","right"],w=["top","bottom"],x=g.px(a,"maxWidth")||a.offsetWidth,B=g.px(a,"maxHeight")||a.offsetHeight;if(x>l.x){b=o;i=0}else if(b+x>o+l.x){i-=t.x;b=a.offsetParent.offsetWidth-(i+g.px(a,"marginRight"));i=1}else{b-=t.x;b-=g.px(a,"marginLeft");i=0}if(B>l.y){e=s;k=0}else if(e+B>s+l.y){if(k>s+l.y)k=s+l.y;k-=t.y;e=a.offsetParent.offsetHeight-(k+g.px(a,"marginBottom"));k=1}else{e-=t.y;e-=g.px(a,"marginTop");k=0}a.style[q[i]]=b+"px";
a.style[q[1-i]]="";a.style[w[k]]=e+"px";a.style[w[1-k]]=""}};this.positionXY=function(a,b,e){a=g.getElement(a);g.isHidden(a)||g.fitToWindow(a,b,e,b,e)};this.Horizontal=1;this.Vertical=2;this.positionAtWidget=function(a,b,e,i,k){a=g.getElement(a);if((b=g.getElement(b))&&a){var l=g.widgetPageCoordinates(b);if(i){a.parentNode.removeChild(a);$(".Wt-domRoot").get(0).appendChild(a)}a.style.position="absolute";if(k)a.style.display="block";if(e==g.Horizontal){e=l.x+b.offsetWidth;i=l.y;k=l.x;b=l.y+b.offsetHeight}else{e=
...
1)B=B.substr(1);var u=B.split("&"),D,C;B="";D=0;for(C=u.length;D<C;++D)if(u[D].split("=")[0]!="_")B+=(B.length?"&":"?")+u[D];return B}var w=q(window.location.search);if(w.length>1){if(w.length>2&&w[0]=="?"&&w[1]=="&")w=w.substr(1);t+="&"+w.substr(1)}}try{window.history.pushState(o?o:"",document.title,t)}catch(x){console.log(x.toString())}g.scrollIntoView(o);s&&i(o)},getCurrentState:function(){return b}}}():function(){function a(){var m,r;r=location.href;m=r.indexOf("#");return m>=0?r.substr(m+1):
null}function b(){u.value=G+"|"+H;if(t)u.value+="|"+p.join(",")}function e(){var m,r;m=0;for(r=V.length;m<r;++m)V[m](unescape(H))}function i(m){if(m){if(!m||H!==m){H=m||G;e()}}else{H=G;e()}}function k(m){var r;m='<html><body><div id="state">'+m+"</div></body></html>";try{r=B.contentWindow.document;r.open();r.write(m);r.close();return true}catch(A){return false}}function l(){var m,r,A,I;if(!B.contentWindow||!B.contentWindow.document)setTimeout(l,10);else{m=B.contentWindow.document;A=(r=m.getElementById("state"))?
r.innerText:null;I=a();setInterval(function(){var E,J;m=B.contentWindow.document;E=(r=m.getElementById("state"))?r.innerText:null;J=a();if(E!==A){A=E;i(A);J=A?A:G;I=location.hash=J;b()}else if(J!==I){I=J;k(J)}},50);D=true;x!=null&&x()}}function o(){if(!q){var m=a(),r=history.length;C&&clearInterval(C);C=setInterval(function(){var A,I;A=a();I=history.length;if(A!==m){m=A;r=I;i(m);b()}else if(I!==r&&t){m=A;r=I;A=p[r-1];i(A);b()}},50)}}function s(){var m;m=u.value.split("|");if(m.length>1){G=m[0];H=
m[1]}else G=H="";if(m.length>2)p=m[2].split(",");if(q)l();else{o();D=true;x!=null&&x()}}var t=false,q=g.isIElt9,w=false,x=null,B=null,u=null,D=false,C=null,p=[],G,H,V=[];return{_initialize:function(){u!=null&&s()},_initTimeout:function(){o()},register:function(m,r){if(!D)H=G=escape(m);V.push(r)},initialize:function(m,r){if(!D){var A=navigator.vendor||"";if(A!=="KDE")if(typeof window.opera!=="undefined")w=true;else if(!q&&A.indexOf("Apple Computer, Inc.")>-1)t=true;if(typeof m==="string")m=document.getElementById(m);
if(!(!m||m.tagName.toUpperCase()!=="TEXTAREA"&&(m.tagName.toUpperCase()!=="INPUT"||m.type!=="hidden"&&m.type!=="text"))){u=m;if(q){if(typeof r==="string")r=document.getElementById(r);!r||r.tagName.toUpperCase()!=="IFRAME"||(B=r)}}}},navigate:function(m,r){m=ga(m);if(D){m=m;if(q)k(m);else{if(m.length>0)location.hash=m;if(t){p[history.length]=m;b()}}r&&e()}},getCurrentState:function(){if(!D)return"";return unescape(H)}}}()});if(window._$_APP_CLASS_$_)try{window._$_APP_CLASS_$_._p_.quit()}catch(e$$28){}
