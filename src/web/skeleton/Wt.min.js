...
"DIV")){b-=a.scrollLeft;f-=a.scrollTop}}while(a!=null&&a!=k)}}return{x:b,y:f}};this.widgetCoordinates=function(a,b){b=g.pageCoordinates(b);a=g.widgetPageCoordinates(a);return{x:b.x-a.x,y:b.y-a.y}};this.pageCoordinates=function(a){if(!a)a=window.event;var b=0,f=0;if(a.pageX||a.pageY){b=a.pageX;f=a.pageY}else if(a.clientX||a.clientY){b=a.clientX+document.body.scrollLeft+document.documentElement.scrollLeft;f=a.clientY+document.body.scrollTop+document.documentElement.scrollTop}return{x:b,y:f}};this.windowCoordinates=
function(a){a=g.pageCoordinates(a);return{x:a.x-document.body.scrollLeft-document.documentElement.scrollLeft,y:a.y-document.body.scrollTop-document.documentElement.scrollTop}};this.scrollIntoView=function(a){(a=document.getElementById(a))&&a.scrollIntoView&&a.scrollIntoView(true)};this.getSelectionRange=function(a){if(document.selection)if(g.hasTag(a,"TEXTAREA")){var b=document.selection.createRange(),f=b.duplicate();f.moveToElementText(a);var k=0;if(b.text.length>1){k-=b.text.length;if(k<0)k=0}a=
-1+k;for(f.moveStart("character",k);f.inRange(b);){f.moveStart("character");a++}b=b.text.replace(/\r/g,"");return{start:a,end:b.length+a}}else{f=$(a).val();a=document.selection.createRange().duplicate();a.moveEnd("character",f.length);b=a.text==""?f.length:f.lastIndexOf(a.text);a=document.selection.createRange().duplicate();a.moveStart("character",-f.length);return{start:b,end:a.text.length}}else return a.selectionStart||a.selectionStart==0?{start:a.selectionStart,end:a.selectionEnd}:{start:-1,end:-1}};
this.setSelectionRange=function(a,b,f){var k=$(a).val();if(typeof b!="number")b=-1;if(typeof f!="number")f=-1;if(b<0)b=0;if(f>k.length)f=k.length;if(f<b)f=b;if(b>f)b=f;a.focus();if(a.selectionStart!=undefined){a.selectionStart=b;a.selectionEnd=f}else if(document.selection){a=a.createTextRange();a.collapse(true);a.moveStart("character",b);a.moveEnd("character",f-b);a.select()}};this.isKeyPress=function(a){if(!a)a=window.event;if(a.altKey||a.ctrlKey||a.metaKey)return false;return(typeof a.charCode!==
"undefined"?a.charCode:0)>0||g.isIE?true:g.isOpera?a.keyCode==13||a.keyCode==27||a.keyCode>=32&&a.keyCode<125:a.keyCode==13||a.keyCode==27||a.keyCode==32||a.keyCode>46&&a.keyCode<112};var Z=/^-?\d+(?:px)?$/i;this.px=function(a,b){a=w(a,b);if(a=="auto"||a==null)return 0;return Z.test(a)?parseFloat(a):0};this.isHidden=function(a){if(a.style.display=="none")return true;else{a=a.parentNode;return a!=null&&a.tagName.toLowerCase()!="body"?g.isHidden(a):false}};this.pxself=function(a,b){a=a.style[b];if(a==
"auto"||a==null)return 0;return(a=(a=/^\s*(-?\d+(?:\.\d+)?)\s*px\s*$/.exec(a))&&a.length==2?a[1]:"0")?parseFloat(a):0};this.pctself=function(a,b){a=a.style[b];if(a=="auto"||a==null)return 0;return(a=(a=/^\s*(-?\d+(?:\.\d+)?)\s*\%\s*$/.exec(a))&&a.length==2?a[1]:"0")?parseFloat(a):0};this.IEwidth=function(a,b,f){if(a.parentNode){var k=a.parentNode.clientWidth-g.px(a,"marginLeft")-g.px(a,"marginRight")-g.px(a,"borderLeftWidth")-g.px(a,"borderRightWidth")-g.px(a.parentNode,"paddingLeft")-g.px(a.parentNode,
...
null)return true;e.sourceId=d.getAttribute("dsid");e.objectPrevStyle={position:e.object.style.position,display:e.object.style.display,left:e.object.style.left,top:e.object.style.top,className:e.object.className};e.object.parentNode.removeChild(e.object);e.object.style.position="absolute";e.object.className="";document.body.appendChild(e.object);m.capture(null);m.capture(e.object);e.object.onmousemove=g;e.object.onmouseup=Z;e.offsetX=-4;e.offsetY=-4;e.dropTarget=null;e.mimeType=d.getAttribute("dmt");
e.xy=m.pageCoordinates(c);m.cancelEvent(c);return false}function g(d){if(aa.object!=null){var c=aa,e=m.pageCoordinates(d);if(c.object.style.display!=""&&c.xy.x!=e.x&&c.xy.y!=e.y)c.object.style.display="";c.object.style.left=e.x-c.offsetX+"px";c.object.style.top=e.y-c.offsetY+"px";e=c.dropTarget;var j=m.target(d),n="{"+c.mimeType+":",i=null;for(c.dropTarget=null;j;){i=j.getAttribute("amts");if(i!=null&&i.indexOf(n)!=-1){c.dropTarget=j;break}j=j.parentNode;if(!j.tagName||m.hasTag(j,"HTML"))break}if(c.dropTarget!=
e){if(c.dropTarget){j=i.indexOf(n)+n.length;var r=i.indexOf("}",j);i=i.substring(j,r);if(i.length!=0){c.dropTarget.setAttribute("dos",c.dropTarget.className);c.dropTarget.className=c.dropTarget.className+" "+i}}else c.object.styleClass="";if(e!=null){e.handleDragDrop&&e.handleDragDrop("end",c.object,d,"",n);i=e.getAttribute("dos");if(i!=null)e.className=i}}if(c.dropTarget)if(c.dropTarget.handleDragDrop)c.dropTarget.handleDragDrop("drag",c.object,d,"",n);else c.object.className="Wt-valid-drop";return false}return true}
function Z(d){m.capture(null);var c=aa;if(c.object){if(c.dropTarget){var e=c.dropTarget.getAttribute("dos");if(e!=null)c.dropTarget.className=e;c.dropTarget.handleDragDrop?c.dropTarget.handleDragDrop("drop",c.object,d,c.sourceId,c.mimeType):I(c.dropTarget,{name:"_drop",eventObject:c.dropTarget,event:d},c.sourceId,c.mimeType)}c.object.style.position=c.objectPrevStyle.position;c.object.style.display=c.objectPrevStyle.display;c.object.style.left=c.objectPrevStyle.left;c.object.style.top=c.objectPrevStyle.top;
c.object.className=c.objectPrevStyle.className;c.object=null}}function B(d,c){var e,j;j=d.event;c=c>0?"&e"+c:"&";e=c+"signal="+d.signal;if(d.id){e+=c+"id="+d.id+c+"name="+encodeURIComponent(d.name)+c+"an="+d.args.length;for(var n=0;n<d.args.length;++n)e+=c+"a"+n+"="+encodeURIComponent(d.args[n])}for(var i=0;i<V.length;++i){var r=m.getElement(V[i]),y=null,fa;if(r!=null){if(r.type=="select-multiple"){n=0;for(fa=r.options.length;n<fa;n++)if(r.options[n].selected)e+=c+V[i]+"="+encodeURIComponent(r.options[n].value)}else if(r.type==
"checkbox"||r.type=="radio")if(r.indeterminate||r.style.opacity=="0.5")y="i";else{if(r.checked)y=r.value}else if(m.hasTag(r,"VIDEO")||m.hasTag(r,"AUDIO"))y=""+r.volume+";"+r.currentTime+";"+r.duration+";"+(r.paused?"1":"0")+";"+(r.ended?"1":"0");else if(r.type!="file"){y=$(r).hasClass("Wt-edit-emptyText")?"":""+r.value;if(m.hasFocus(r)){n=m.getSelectionRange(r);e+=c+"selstart="+n.start+c+"selend="+n.end}}if(y!=null)e+=c+V[i]+"="+encodeURIComponent(y)}}try{if(document.activeElement)e+=c+"focus="+document.activeElement.id}catch(ma){}if(O!=
null)e+=c+"_="+encodeURIComponent(unescape(O));if(!j){d.data=e;return d}for(i=m.target(j);!i.id&&i.parentNode;)i=i.parentNode;if(i.id)e+=c+"tid="+i.id;try{e+=c+"type="+j.type}catch(na){}if(j.clientX||j.clientY)e+=c+"clientX="+j.clientX+c+"clientY="+j.clientY;y=m.pageCoordinates(j);i=y.x;y=y.y;if(i||y){e+=c+"documentX="+i+c+"documentY="+y;e+=c+"dragdX="+(i-N)+c+"dragdY="+(y-J);n=0;if(j.wheelDelta)n=j.wheelDelta>0?1:-1;else if(j.detail)n=j.detail<0?1:-1;e+=c+"wheel="+n}if(j.screenX||j.screenY)e+=c+
"screenX="+j.screenX+c+"screenY="+j.screenY;if(d.object&&d.object.nodeType!=9){n=m.widgetPageCoordinates(d.object);e+=c+"scrollX="+d.object.scrollLeft+c+"scrollY="+d.object.scrollTop+c+"width="+d.object.clientWidth+c+"height="+d.object.clientHeight+c+"widgetX="+(i-n.x)+c+"widgetY="+(y-n.y)}i=m.button(j);if(!i)if(m.buttons&1)i=1;else if(m.buttons&2)i=2;else if(m.buttons&4)i=4;e+=c+"button="+i;if(typeof j.keyCode!=="undefined")e+=c+"keyCode="+j.keyCode;if(typeof j.charCode!=="undefined")e+=c+"charCode="+
j.charCode;if(j.altKey)e+=c+"altKey=1";if(j.ctrlKey)e+=c+"ctrlKey=1";if(j.metaKey)e+=c+"metaKey=1";if(j.shiftKey)e+=c+"shiftKey=1";d.data=e;return d}function S(){for(var d="",c=false,e=0;e<A.length;++e){c=c||A[e].feedback;d+=A[e].data}da=A;A=[];return{feedback:c,result:d}}function Y(){P=true;if(W){clearTimeout(W);W=null}var d=$("#Wt-timers");d.size()>0&&m.setHtml(d.get(0),"",false)}function a(){m.history._initTimeout();X==0&&z(null,"none",null,false);W=setTimeout(a,_$_KEEP_ALIVE_$_000)}function b(d){if(!m.isIEMobile)document.title=
d}function f(){if(!document.activeElement){function d(e){if(e&&e.target)document.activeElement=e.target==document?null:e.target}function c(){document.activeElement=null}document.addEventListener("focus",d,true);document.addEventListener("blur",c,true)}m.history._initialize();T();if(!ga){ga=true;_$_ONLOAD_$_();P||(W=setTimeout(a,_$_KEEP_ALIVE_$_000))}}function k(d){clearTimeout(d);document.body.style.cursor="auto";if(ba!=null){try{ba()}catch(c){}ba=null}}function h(){document.body.style.cursor="wait";
ba=hideLoadingIndicator;showLoadingIndicator()}function o(d){ha=d}function E(d){if(d){d="(function() {"+d+"})();";window.execScript?window.execScript(d):window.eval(d)}q._p_.autoJavaScript()}function u(d,c,e){if(!P){if(d==0){_$_$ifnot_DEBUG_$_();try{_$_$endif_$_();E(c);_$_$ifnot_DEBUG_$_()}catch(j){alert("Wt internal error: "+j+", code: "+j.code+", description: "+j.description)}_$_$endif_$_();e&&k(e)}else A=da.concat(A);da=[];if(L){clearTimeout(L);L=null}K=null;if(d>0)++X;else X=0;if(!P)if(ha||A.length>
