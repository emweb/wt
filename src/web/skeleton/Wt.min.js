...
window._$_WT_CLASS_$_=new (function(){function w(a,b){return a.style[b]?a.style[b]:document.defaultView&&document.defaultView.getComputedStyle?document.defaultView.getComputedStyle(a,null)[b]:a.currentStyle?a.currentStyle[b]:null}function v(a,b,f){if(a=="auto"||a==null)return f;return(a=(a=b.exec(a))&&a.length==2?a[1]:null)?parseFloat(a):f}function G(a){return v(a,/^\s*(-?\d+(?:\.\d+)?)\s*px\s*$/i,0)}function U(a,b){return v(a,/^\s*(-?\d+(?:\.\d+)?)\s*\%\s*$/i,b)}function ca(a){if(z==null)return null;
...
g.button(a)};this.mouseUp=function(a){g.buttons^=g.button(a)};this.arrayRemove=function(a,b,f){f=a.slice((f||b)+1||a.length);a.length=b<0?a.length+b:b;return a.push.apply(a,f)};var X=function(){for(var a,b=3,f=document.createElement("div"),l=f.getElementsByTagName("i");f.innerHTML="<!--[if gt IE "+ ++b+"]><i></i><![endif]--\>",l[0];);return b>4?b:a}();this.isIE=X!==undefined;this.isIE6=X===6;this.isIElt9=X<9;this.isGecko=navigator.userAgent.toLowerCase().indexOf("gecko")!=-1;this.isIEMobile=navigator.userAgent.toLowerCase().indexOf("msie 4")!=
-1||navigator.userAgent.toLowerCase().indexOf("msie 5")!=-1;this.isOpera=typeof window.opera!=="undefined";this.updateDelay=this.isIE?10:51;this.setHtml=function(a,b,f){function l(o,L){var u,q,B;switch(o.nodeType){case 1:u=o.namespaceURI==null?document.createElement(o.nodeName):document.createElementNS(o.namespaceURI,o.nodeName);if(o.attributes&&o.attributes.length>0){q=0;for(B=o.attributes.length;q<B;)u.setAttribute(o.attributes[q].nodeName,o.getAttribute(o.attributes[q++].nodeName))}if(L&&o.childNodes.length>
0){q=0;for(B=o.childNodes.length;q<B;){var r=l(o.childNodes[q++],L);r&&u.appendChild(r)}}return u;case 3:case 4:case 5:return document.createTextNode(o.nodeValue)}return null}if(g.isIE||_$_INNER_HTML_$_&&!f)if(f)a.innerHTML+=b;else a.innerHTML=b;else{var n;n=new DOMParser;n=n.parseFromString("<div>"+b+"</div>","application/xhtml+xml").documentElement;if(n.nodeType!=1)n=n.nextSibling;if(!f)a.innerHTML="";b=0;for(f=n.childNodes.length;b<f;)a.appendChild(l(n.childNodes[b++],true))}};this.hasTag=function(a,
...
{position:e.object.style.position,display:e.object.style.display,left:e.object.style.left,top:e.object.style.top,className:e.object.className};e.object.parentNode.removeChild(e.object);e.object.style.position="absolute";e.object.className="";document.body.appendChild(e.object);h.capture(null);h.capture(e.object);e.object.onmousemove=V;e.object.onmouseup=R;e.offsetX=-4;e.offsetY=-4;e.dropTarget=null;e.mimeType=c.getAttribute("dmt");e.xy=h.pageCoordinates(d);h.cancelEvent(d);return false}function V(c){if(E.object!=
null){var d=E,e=h.pageCoordinates(c);if(d.object.style.display!=""&&d.xy.x!=e.x&&d.xy.y!=e.y)d.object.style.display="";d.object.style.left=e.x-d.offsetX+"px";d.object.style.top=e.y-d.offsetY+"px";e=d.dropTarget;var j=h.target(c),m="{"+d.mimeType+":",i=null;for(d.dropTarget=null;j;){i=j.getAttribute("amts");if(i!=null&&i.indexOf(m)!=-1){d.dropTarget=j;break}j=j.parentNode;if(!j.tagName||h.hasTag(j,"HTML"))break}if(d.dropTarget!=e){if(d.dropTarget){j=i.indexOf(m)+m.length;var p=i.indexOf("}",j);i=i.substring(j,
p);if(i.length!=0){d.dropTarget.setAttribute("dos",d.dropTarget.className);d.dropTarget.className=d.dropTarget.className+" "+i}}else d.object.styleClass="";if(e!=null){e.handleDragDrop&&e.handleDragDrop("end",d.object,c,"",m);i=e.getAttribute("dos");if(i!=null)e.className=i}}if(d.dropTarget)if(d.dropTarget.handleDragDrop)d.dropTarget.handleDragDrop("drag",d.object,c,"",m);else d.object.className="Wt-valid-drop";return false}return true}function R(c){h.capture(null);var d=E;if(d.object){if(d.dropTarget){var e=
...
0;for(ia=p.options.length;m<ia;m++)if(p.options[m].selected)e+=d+Z[i]+"="+encodeURIComponent(p.options[m].value)}else if(p.type=="checkbox"||p.type=="radio")if(p.indeterminate||p.style.opacity=="0.5")t="i";else{if(p.checked)t=p.value}else if(h.hasTag(p,"VIDEO")||h.hasTag(p,"AUDIO"))t=""+p.volume+";"+p.currentTime+";"+p.duration+";"+(p.paused?"1":"0")+";"+(p.ended?"1":"0");else if(p.type!="file"){t=$(p).hasClass("Wt-edit-emptyText")?"":""+p.value;if(h.hasFocus(p)){m=h.getSelectionRange(p);e+=d+"selstart="+
m.start+d+"selend="+m.end}}if(t!=null)e+=d+Z[i]+"="+encodeURIComponent(t)}}try{if(document.activeElement)e+=d+"focus="+document.activeElement.id}catch(oa){}if(H!=null)e+=d+"_="+encodeURIComponent(unescape(H));if(!j){c.data=e;return c}for(i=h.target(j);!i.id&&i.parentNode;)i=i.parentNode;if(i.id)e+=d+"tid="+i.id;try{e+=d+"type="+j.type}catch(pa){}if(j.clientX||j.clientY)e+=d+"clientX="+j.clientX+d+"clientY="+j.clientY;t=h.pageCoordinates(j);i=t.x;t=t.y;if(i||t){e+=d+"documentX="+i+d+"documentY="+t;
e+=d+"dragdX="+(i-s)+d+"dragdY="+(t-A);m=h.wheelDelta(j);e+=d+"wheel="+m}if(j.screenX||j.screenY)e+=d+"screenX="+j.screenX+d+"screenY="+j.screenY;m={x:0,y:0};if(c.object&&c.object.nodeType!=9){m=h.widgetPageCoordinates(c.object);e+=d+"scrollX="+c.object.scrollLeft+d+"scrollY="+c.object.scrollTop+d+"width="+c.object.clientWidth+d+"height="+c.object.clientHeight+d+"widgetX="+(i-m.x)+d+"widgetY="+(t-m.y)}i=h.button(j);if(!i)if(h.buttons&1)i=1;else if(h.buttons&2)i=2;else if(h.buttons&4)i=4;e+=d+"button="+
i;if(typeof j.keyCode!=="undefined")e+=d+"keyCode="+j.keyCode;if(typeof j.charCode!=="undefined")e+=d+"charCode="+j.charCode;if(j.altKey)e+=d+"altKey=1";if(j.ctrlKey)e+=d+"ctrlKey=1";if(j.metaKey)e+=d+"metaKey=1";if(j.shiftKey)e+=d+"shiftKey=1";if(typeof j.touches!=="undefined")e+=W(d+"touches",j.touches,m);if(typeof j.targetTouches!=="undefined")e+=W(d+"ttouches",j.targetTouches,m);if(typeof j.changedTouches!=="undefined")e+=W(d+"ctouches",j.changedTouches,m);if(j.scale)e+=d+"scale="+j.scale;if(j.rotation)e+=
...
e.target==document?null:e.target}function d(){document.activeElement=null}document.addEventListener("focus",c,true);document.addEventListener("blur",d,true)}h.history._initialize();U();if(!ja){ja=true;_$_ONLOAD_$_();P||(aa=setTimeout(z,_$_KEEP_ALIVE_$_000))}}function b(c){clearTimeout(c);document.body.style.cursor="auto";if(ea!=null){try{ea()}catch(d){}ea=null}}function f(){document.body.style.cursor="wait";ea=hideLoadingIndicator;showLoadingIndicator()}function l(c){ka=c}function n(c){if(c){c="(function() {"+
c+"})();";window.execScript?window.execScript(c):window.eval(c)}k._p_.autoJavaScript()}function o(c,d,e){if(!P){if(c==0){_$_$ifnot_DEBUG_$_();try{_$_$endif_$_();n(d);_$_$ifnot_DEBUG_$_()}catch(j){alert("Wt internal error: "+j+", code: "+j.code+", description: "+j.description)}_$_$endif_$_();e&&b(e)}else y=ga.concat(y);ga=[];if(Q){clearTimeout(Q);Q=null}F=null;if(c>0)++ba;else ba=0;if(!P)if(ka||y.length>0)if(c==1){c=Math.min(12E4,Math.exp(ba)*500);S=setTimeout(function(){r()},c)}else r()}}function L(){F.abort();
...
"//"+location.hostname+":"+location.port+location.pathname+c;I.socket=c=new WebSocket(c);c.onmessage=function(d){I.state=1;o(0,d.data,null)}}if(c.readyState==1){r();return}}_$_$endif_$_();if(F!=null&&Q!=null){clearTimeout(Q);F.abort();F=null}if(F==null)if(S==null){S=setTimeout(function(){r()},h.updateDelay);la=(new Date).getTime()}else if(ba){clearTimeout(S);r()}else if((new Date).getTime()-la>h.updateDelay){clearTimeout(S);r()}}function B(c){fa=c;k._p_.comm.responseReceived(c)}function r(){if(!F){S=
...
true;return"0"}function Y(c){window.onbeforeunload=c&&c!=""?function(d){if(d=d||window.event)d.returnValue=c;return c}:null}var k=this,h=_$_WT_CLASS_$_,s=0,A=0,H=null,E={object:null,sourceId:null,mimeType:null,dropOffsetX:null,dragOffsetY:null,dropTarget:null,objectPrevStyle:null,xy:null},Z=_$_FORM_OBJECTS_$_;h.button=function(c){return c.which?c.which==3?4:c.which==2?2:1:h.isIE&&typeof c.button!="undefined"?c.button==2?4:c.button==4?2:1:typeof c.button!="undefined"?c.button==2?4:c.button==1?2:1:
...
function(){if(!P){k.emit(k,"Wt-unload");r()}};this._p_={ieAlternative:D,loadScript:O,onJsLoad:K,setTitle:da,update:u,quit:N,setFormObjects:function(c){Z=c},saveDownPos:w,addTimerEvent:J,load:a,handleResponse:o,setServerPush:l,dragStart:ca,dragDrag:V,dragEnd:R,capture:h.capture,onHashChange:v,setHash:G,ImagePreloader:x,autoJavaScript:function(){_$_AUTO_JAVASCRIPT_$_()},response:B,setCloseMessage:Y};this.WT=_$_WT_CLASS_$_;this.emit=C});window.WtSignalEmit=_$_APP_CLASS_$_.emit;
