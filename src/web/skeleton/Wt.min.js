...
null;f=false}else{d={result:"signal=poll"};c=null;f=true}J=y._p_.comm.sendUpdate(ka+j,"request=jsupdate&"+d.result+"&ackId="+da,c,da,-1);O=f?setTimeout(x,_$_SERVER_PUSH_TIMEOUT_$_):null}}function M(d,c){var f={},j=z.length;f.signal="user";f.id=typeof d==="string"?d:d==_$_APP_CLASS_$_?"app":d.id;if(typeof c==="object"){f.name=c.name;f.object=c.eventObject;f.event=c.event}else{f.name=c;f.object=f.event=null}f.args=[];for(var m=2;m<arguments.length;++m){var g=arguments[m];g=g===false?0:g===true?1:g.toDateString?
g.toDateString():g;f.args[m-2]=g}f.feedback=true;z[j]=T(f,j);F()}function L(d,c,f){var j=function(){var g=o.getElement(d);if(g){if(f)g.timer=setTimeout(g.tm,c);else{g.timer=null;g.tm=null}g.onclick&&g.onclick()}},m=o.getElement(d);m.timer=setTimeout(j,c);m.tm=j}function E(d,c){setTimeout(function(){if(S[d]===true)c();else S[d]=c},20)}function N(d){if(S[d]!==true){typeof S[d]!=="undefined"&&S[d]();S[d]=true}}function l(d,c){var f=false;if(c!="")try{f=!eval("typeof "+c+" === 'undefined'")}catch(j){f=
...
"";d.ieAlternativeExecuted=true;return"0"}var y=this,o=_$_WT_CLASS_$_,H=0,ea=0,P=null,Y={object:null,sourceId:null,mimeType:null,dropOffsetX:null,dragOffsetY:null,dropTarget:null,objectPrevStyle:null,xy:null},U=_$_FORM_OBJECTS_$_;o.button=function(d){return d.which?d.which==3?4:d.which==2?2:1:o.isIE&&typeof d.button!="undefined"?d.button==2?4:d.button==4?2:1:typeof d.button!="undefined"?d.button==2?4:d.button==1?2:1:0};var ca=[],z=[],ka=_$_RELATIVE_URL_$_,Q=false,ia=false,fa=false,J=null,O=null,V=
...
addTimerEvent:L,load:i,handleResponse:D,setServerPush:u,dragStart:h,dragDrag:X,dragEnd:C,capture:o.capture,onHashChange:t,setHash:B,ImagePreloader:p,autoJavaScript:function(){_$_AUTO_JAVASCRIPT_$_()},response:I};this.WT=_$_WT_CLASS_$_;this.emit=M}),WtSignalEmit=_$_APP_CLASS_$_.emit;window.WtScriptLoaded=false;function onLoad(){if(window.WtScriptLoaded){_$_WT_CLASS_$_.history.initialize("Wt-history-field","Wt-history-iframe");_$_APP_CLASS_$_._p_.load()}else window.isLoaded=true};
