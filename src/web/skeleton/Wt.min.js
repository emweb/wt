/*
 ...
 version: 2.5.2
*/
_$_$if_DYNAMIC_JS_$_();window.JavaScriptFunction=1;window.JavaScriptConstructor=2;window.JavaScriptObject=3;window.JavaScriptPrototype=4;window.WT_DECLARE_WT_MEMBER=function(P,Q,I,N){if(Q==JavaScriptPrototype){P=I.indexOf(".prototype");_$_WT_CLASS_$_[I.substr(0,P)].prototype[I.substr(P+11)]=N}else _$_WT_CLASS_$_[I]=Q==JavaScriptFunction?function(){return N.apply(_$_WT_CLASS_$_,arguments)}:N};
window.WT_DECLARE_APP_MEMBER=function(P,Q,I,N){var U=window.currentApp;if(Q==JavaScriptPrototype){P=I.indexOf(".prototype");U[I.substr(0,P)].prototype[I.substr(P+11)]=N}else U[I]=Q==JavaScriptFunction?function(){return N.apply(U,arguments)}:N};_$_$endif_$_();
...
0){t=0;for(u=m.attributes.length;t<u;)p.setAttribute(m.attributes[t].nodeName,m.getAttribute(m.attributes[t++].nodeName))}if(o&&m.childNodes.length>0){t=0;for(u=m.childNodes.length;t<u;){var v=g(m.childNodes[t++],o);v&&p.appendChild(v)}}return p;case 3:case 4:case 5:return document.createTextNode(m.nodeValue)}return null}if(h.isIE||_$_INNER_HTML_$_&&!c)if(c)a.innerHTML+=b;else{h.saveReparented(a);a.innerHTML=b}else{var k;k=new DOMParser;k=k.parseFromString("<div>"+b+"</div>","application/xhtml+xml").documentElement;
...
if(v==null){v=o.location.pathname.lastIndexOf(b);if(v!=-1&&v==o.location.pathname.length-b.length){a(b);return}else v=o.location.pathname.substr(c.length)}if(v!=b){b=v;t(b!=""?b:"/")}}b=p;k=t;a(p);o.addEventListener("popstate",u,false)},initialize:function(p,t,u){h.resolveRelativeAnchors();c=u;if(c.length>=1&&c[c.length-1]=="/"){_$_$if_UGLY_INTERNAL_PATHS_$_();g=true;_$_$endif_$_();_$_$ifnot_UGLY_INTERNAL_PATHS_$_();c=c.substr(0,c.length-1);_$_$endif_$_()}},navigate:function(p,t){h.resolveRelativeAnchors();
...
0&&b(null,"none",null,false)}function ga(d){if(!r.isIEMobile)document.title=d}function Z(d){if(!Ga){if(d){if(!window._$_APP_CLASS_$_LoadWidgetTree)return;r.history.initialize("Wt-history-field","Wt-history-iframe",K)}if(!("activeElement"in document)){function e(l){if(l&&l.target)document.activeElement=l.target==document?null:l.target}function f(){document.activeElement=null}document.addEventListener("focus",e,true);document.addEventListener("blur",f,true)}$(document).mousedown(r.mouseDown).mouseup(r.mouseUp);
r.history._initialize();N();Ga=true;d&&window._$_APP_CLASS_$_LoadWidgetTree();X||va||(va=setInterval(sa,_$_KEEP_ALIVE_$_000))}}function ha(d){clearTimeout(d);document.body.style.cursor="auto";if(Ba!=null){try{Ba()}catch(e){}Ba=null}}function fa(){document.body.style.cursor="wait";Ba=hideLoadingIndicator;showLoadingIndicator()}function da(d){Ha=d}function ea(){w._p_.autoJavaScript()}function ta(d){if(d){d="(function() {"+d+"})();";window.execScript?window.execScript(d):window.eval(d)}w==window._$_APP_CLASS_$_&&
ea()}function ia(){y.socket.send("&signal=none&connected="+xa);y.state=3}function J(d,e,f){if(!X)if(Ca)setTimeout(function(){J(d,e,f)},50);else{if(d==0){r.resolveRelativeAnchors();_$_$if_CATCH_ERROR_$_();try{_$_$endif_$_();ta(e);_$_$if_CATCH_ERROR_$_()}catch(l){var q=l.description||l.message,n={exception_code:l.code,exception_description:q,exception_js:e};n.stack=l.stack||l.stacktrace;o(n,"Wt internal error; code: "+l.code+", description: "+q);throw l;}_$_$endif_$_();f&&ha(f)}else M=Da.concat(M);
...
l,f)}}}function b(d,e,f,l){if(!Ea){Ea=true;r.checkReleaseCapture(d,f);_$_$if_STRICTLY_SERIALIZED_EVENTS_$_();if(!G){_$_$endif_$_();var q={},n=M.length;q.object=d;q.signal=e;q.event=window.fakeEvent||f;q.feedback=l;M[n]=ra(q,n);g();ta();_$_$if_STRICTLY_SERIALIZED_EVENTS_$_()}_$_$endif_$_();Ea=false}}function c(){y.keepAlive&&clearInterval(y.keepAlive);y.keepAlive=setInterval(function(){var d=y.socket;if(d.readyState==1)d.send("&signal=ping");else{clearInterval(y.keepAlive);y.keepAlive=null}},_$_SERVER_PUSH_TIMEOUT_$_)}
function g(){if(X){if(Aa){if(confirm(Aa))document.location=document.location;Aa=null}}else{_$_$if_WEB_SOCKETS_$_();if(y.state!=4)if(typeof window.WebSocket==="undefined"&&typeof window.MozWebSocket==="undefined")y.state=4;else{var d=y.socket;if(d==null||d.readyState>1)if(d!=null&&y.state==0)y.state=4;else{function e(){if(!X){++y.reconnectTries;var l=Math.min(12E4,Math.exp(y.reconnectTries)*500);setTimeout(function(){g()},l)}}var f;if(la.indexOf("://")!=-1)f="ws"+la.substr(4);else{f=la.substr(la.indexOf("?"));
f="ws"+location.protocol.substr(4)+"//"+location.host+K+f}f+="&request=ws";y.socket=typeof window.WebSocket!=="undefined"?(d=new WebSocket(f)):(d=new MozWebSocket(f));y.state=1;y.keepAlive&&clearInterval(y.keepAlive);y.keepAlive=null;d.onmessage=function(l){var q=null;if(y.state==1)if(l.data=="connect"){if(G!=null&&Y!=null){clearTimeout(Y);G.abort();G=null}if(G)y.state=2;else ia()}else{console.log("WebSocket: was expecting a connect?");return}else{if(H){H.onStatusChange("websocket",true);H.onStatusChange("connectionStatus",
1)}y.state=3;q=l.data}y.reconnectTries=0;q!=null&&J(0,q,null)};d.onerror=function(){H&&H.onStatusChange("websocket",false);if(y.reconnectTries==3&&y.state==0)y.state=4;e()};d.onclose=function(){H&&H.onStatusChange("websocket",false);if(y.reconnectTries==3&&y.state==0)y.state=4;e()};d.onopen=function(){if(H){H.onStatusChange("websocket",true);H.onStatusChange("connectionStatus",1)}c()}}if(d.readyState==1&&d.state==3){c();p();return}}_$_$endif_$_();if(G!=null&&Y!=null){clearTimeout(Y);G.abort();G=null}if(G==
null)if(ka==null){ka=setTimeout(function(){p()},r.updateDelay);Ia=(new Date).getTime()}else if(wa){clearTimeout(ka);p()}else if((new Date).getTime()-Ia>r.updateDelay){clearTimeout(ka);p()}}}function k(d,e){Fa=e;xa=d;ma.responseReceived(d)}function m(d){Ja=d}function o(d,e){G=ma.sendUpdate("request=jserror&err="+encodeURIComponent(JSON.stringify(d)),false,xa,-1);_$_$if_SHOW_ERROR_$_();alert(e);_$_$endif_$_()}function p(){if(w!=window._$_APP_CLASS_$_)ca(null);else if(!G){ka=null;if(!X){var d,e,f;if(M.length>
0){d=h();e=d.feedback?setTimeout(fa,_$_INDICATOR_TIMEOUT_$_):null;f=false}else{d={result:"&signal=poll"};e=null;f=true}d.result+="&ackId="+xa+"&pageId="+Ja;if(Fa){var l="",q=$("#"+Fa).get(0);if(q)for(q=q.parentNode;!r.hasTag(q,"BODY");q=q.parentNode)if(q.id){if(l!="")l+=",";l+=q.id}d.result+="&ackPuzzle="+encodeURIComponent(l)}d.result+="&_$_PARAMS_$_";if(y.socket!=null&&y.socket.readyState==1&&y.state==3){G=null;e!=null&&clearTimeout(e);f||y.socket.send(d.result)}else{G=ma.sendUpdate("request=jsupdate"+
d.result,e,xa,-1);Y=f?setTimeout(ba,_$_SERVER_PUSH_TIMEOUT_$_):null}}}}function t(d,e,f){if(e==-1)e=d.offsetWidth;if(f==-1)f=d.offsetHeight;if(typeof d.wtWidth==="undefined"||d.wtWidth!=e||typeof d.wtHeight==="undefined"||d.wtHeight!=f){d.wtWidth=e;d.wtHeight=f;e>=0&&f>=0&&u(d,"resized",Math.round(e),Math.round(f))}}function u(d,e){var f={},l=M.length;f.signal="user";f.id=typeof d==="string"?d:d==w?"app":d.id;if(typeof e==="object"){f.name=e.name;f.object=e.eventObject;f.event=e.event}else{f.name=
...
d;r.history.register(d,Q)}function O(d){if(d.ieAlternativeExecuted)return"0";w.emit(d.parentNode,"IeAlternative");d.style.width="";d.ieAlternativeExecuted=true;return"0"}function V(d){if(!(r.isIEMobile||d==""))document.documentElement.lang=d}function s(d){window.onbeforeunload=d&&d!=""?function(e){if(e=e||window.event)e.returnValue=d;return d}:null}var w=this,r=_$_WT_CLASS_$_,T=0,ja=0,K=_$_DEPLOY_PATH_$_,aa=null,za={object:null,sourceId:null,mimeType:null,dropOffsetX:null,dragOffsetY:null,dropTarget:null,
objectPrevStyle:null,xy:null},ua=[],Da=[],M=[],la,X=false,Aa=_$_QUITTED_STR_$_,Ga=false,G=null,Y=null,va=null,wa=0,Ha=false,ka=null,Ba=null,y={state:0,socket:null,keepAlive:null,reconnectTries:0},H=null;pa(_$_SESSION_URL_$_);var ma=r.initAjaxComm(la,J),Ea=false,Ia,xa=_$_ACK_UPDATE_ID_$_,Fa=null,Ja=0,na={},Ca=false;x.prototype.preload=function(d){var e=new Image;this.images.push(e);e.onload=x.prototype.onload;e.onerror=x.prototype.onload;e.onabort=x.prototype.onload;e.imagePreloader=this;e.src=d};
...
response:k,setPage:m,setCloseMessage:s,setConnectionMonitor:a,propagateSize:t};this.WT=_$_WT_CLASS_$_;this.emit=u});window._$_APP_CLASS_$_SignalEmit=_$_APP_CLASS_$_.emit;window._$_APP_CLASS_$_OnLoad=function(){_$_APP_CLASS_$_._p_.load()};
