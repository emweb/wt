...
u=null;if(s!=null){if(s.type=="select-multiple")for(d=0;d<s.options.length;d++){if(s.options[d].selected)g+=e+Q[i]+"="+encodeURIComponent(s.options[d].value)}else if(q.hasTag(s,"SPAN"))for(d=0;d<s.childNodes.length;d++){if(s.childNodes[d].type=="checkbox"){d=s.childNodes[d];if(d.style.display=="none")u="indeterminate";else if(d.checked)u=d.value;break}}else if(s.type=="checkbox"||s.type=="radio")if(s.indeterminate)u="indeterminate";else{if(s.checked)u=s.value}else if(s.type!="file")u=""+s.value;if(u!=
null)g+=e+Q[i]+"="+encodeURIComponent(u)}}if(L!=null)g+=e+"_="+encodeURIComponent(unescape(L));if(!h){c.data=g;return c}for(i=h.target||h.srcElement;!i.id&&i.parentNode;)i=i.parentNode;if(i.id)g+=e+"tid="+i.id;try{g+=e+"type="+h.type}catch(ia){}if(h.clientX||h.clientY)g+=e+"clientX="+h.clientX+e+"clientY="+h.clientY;u=q.pageCoordinates(h);i=u.x;u=u.y;if(i||u){g+=e+"documentX="+i+e+"documentY="+u;g+=e+"dragdX="+(i-$)+e+"dragdY="+(u-aa)}if(h.screenX||h.screenY)g+=e+"screenX="+h.screenX+e+"screenY="+
...
h.setAttribute("src",c);h.onload=function(){Z(c)};h.onreadystatechange=function(){if(h.readyState=="complete"||h.readyState=="loaded")Z(c)};document.getElementsByTagName("head")[0].appendChild(h)}}function N(c,d){this.callback=d;this.work=c.length;this.images=[];if(c.length==0)d(this.images);else for(d=0;d<c.length;d++)this.preload(c[d])}var da=this,q=_$_WT_CLASS_$_,$=0,aa=0,L=null,T={object:null,sourceId:null,mimeType:null,dropOffsetX:null,dragOffsetY:null,dropTarget:null,objectPrevStyle:null},Q=
_$_FORM_OBJECTS_$_,V=[],v=[],ga=_$_RELATIVE_URL_$_,X=false,ea=false,ba=false,C=null,K=null,W=null,R=0,ca=false,S=null,U=null,fa=(new Date).getTime(),H=null,Y=0,M={};N.prototype.preload=function(c){var d=new Image;this.images.push(d);d.onload=N.prototype.onload;d.onerror=N.prototype.onload;d.onabort=N.prototype.onload;d.imagePreloader=this;d.src=c};N.prototype.onload=function(){var c=this.imagePreloader;--c.work==0&&c.callback(c.images)};q.history.register(_$_INITIAL_HASH_$_,a);this._p_={loadScript:ha,
onJsLoad:G,setTitle:z,update:J,quit:function(){X=true;clearTimeout(W)},setFormObjects:function(c){Q=c},saveDownPos:l,addTimerEvent:O,load:F,handleResponse:D,setServerPush:P,dragStart:k,dragDrag:m,dragEnd:x,capture:f,onHashChange:a,setHash:b,ImagePreloader:N,autoJavaScript:function(){with(window)_$_AUTO_JAVASCRIPT_$_()},response:o};this.emit=w}),WtSignalEmit=_$_APP_CLASS_$_.emit;window.WtScriptLoaded=false;
