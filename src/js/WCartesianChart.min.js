...
false;r=[E.widgetCoordinates(A.canvas,b.touches[0]),E.widgetCoordinates(A.canvas,b.touches[1])].map(function(j){return[j.x,j.y]});f=false;a=-1;if(!r.every(function(j){return Ma(j,h())})){(f=Na(r[0][v],r[0][u])&&Na(r[1][v],r[1][u]))||(a=Oa(r[0][v],r[0][u]));if(!f&&(a===-1||Oa(r[1][v],r[1][u])!==a)){ra=null;return}G=f;t=a}E.capture(null);E.capture(vb());U=Math.atan2(r[1][1]-r[0][1],r[1][0]-r[0][0]);hb=[(r[0][0]+r[1][0])/2,(r[0][1]+r[1][1])/2];c=Math.abs(Math.sin(U));d=Math.abs(Math.cos(U));U=c<Math.sin(0.125*
Math.PI)?0:d<Math.cos(0.375*Math.PI)?Math.PI/2:Math.tan(U)>0?Math.PI/4:-Math.PI/4;ib=Jb(U,hb);G=f;t=a}else return;ya(b)};aa.end=function(a,b){if(ba){window.clearTimeout(ba);ba=null}window.setTimeout(function(){W("contextmenu",ta.contextmenuListener)},0);var c=Array.prototype.slice.call(b.touches),d=y(c)===0;d||function(){var e;for(e=0;e<y(b.changedTouches);++e)(function(){for(var f=b.changedTouches[e].identifier,j=0;j<y(c);++j)if(c[j].identifier===f){c.splice(j,1);return}})()}();d=y(c)===0;ja=y(c)===
1;ra=y(c)===2;if(d){$a=null;if(Da===0&&(isFinite(k.x)||isFinite(k.y))&&l.rubberBand){oa=Date.now();Ba=true;Xa(eb)}else{Da===1&&ia.mouseUp(null,null);c=[];oa=ib=hb=U=null}Da=null}else if(ja||ra)aa.start(a,b,true)};var $a=null,va=null,jb=null;aa.moved=function(a,b){if(ja||ra)if(!(ja&&D==null)){ya(b);va=E.widgetCoordinates(A.canvas,b.touches[0]);if(y(b.touches)>1)jb=E.widgetCoordinates(A.canvas,b.touches[1]);if(!G&&t===-1&&ja&&ba&&!Ab([va.x,va.y],[D.x,D.y],3)){window.clearTimeout(ba);ba=null}$a||($a=
setTimeout(function(){if(!G&&t===-1&&ja&&Ga()&&ca(fa())){var c=fa();if(ca(c)){var d=va,e;e=B()?(d.x-D.x)/g(P(fa()))[3]:(d.y-D.y)/g(P(fa()))[3];da(c)[5]+=e;D=d;Y()}}else if(ja){d=va;e=Date.now();var f={x:d.x-D.x,y:d.y-D.y};c=e-oa;oa=e;if(Da===1){C[v]+=f.x;C[u]+=f.y;xa()&&na&&Xa(db)}else if(l.pan){k.x=f.x/c;k.y=f.y/c;Z(f,l.rubberBand?zb:0,t,G)}D=d}else if(!G&&t===-1&&ra&&Ga()&&ca(fa())){f=B()?v:u;e=[va,jb].map(function(S){return B()?[S.x,sa]:[Ea,S.y]});c=Math.abs(r[1][f]-r[0][f]);var j=Math.abs(e[1][f]-
e[0][f]),m=c>0?j/c:1;if(j===c)m=1;c=fa();if(ca(c)){var sa=w(Ia(la(P(c))),[0,(r[0][f]+r[1][f])/2])[1],Fa=w(Ia(la(P(c))),[0,(e[0][f]+e[1][f])/2])[1];ka(da(c),w([1,0,0,m,0,-m*sa+Fa],da(c)));D=d;Y();r=e}}else if(ra&&l.zoom){d=ma(C,Q());var Ea=(r[0][0]+r[1][0])/2;sa=(r[0][1]+r[1][1])/2;e=[va,jb].map(function(S){return U===0?[S.x,sa]:U===Math.PI/2?[Ea,S.y]:w(ib,[S.x,S.y])});f=Math.abs(r[1][0]-r[0][0]);c=Math.abs(e[1][0]-e[0][0]);var O=f>0?c/f:1;if(c===f||U===Math.PI/2)O=1;var ab=(e[0][0]+e[1][0])/2;c=Math.abs(r[1][1]-
r[0][1]);j=Math.abs(e[1][1]-e[0][1]);m=c>0?j/c:1;if(j===c||U===0)m=1;Fa=(e[0][1]+e[1][1])/2;B()&&function(){var S=O;O=m;m=S;S=ab;ab=Fa;Fa=S;S=Ea;Ea=sa;sa=S}();if(i()[0]*O>ea())O=ea()/i()[0];if(i()[0]*O<Qa())O=Qa()/i()[0];f=[];for(c=0;c<M();++c)f.push(m);for(c=0;c<M();++c){if(g(c)[3]*f[c]>T(c))f[c]=T(c)/g(c)[3];if(g(c)[3]*f[c]<pb(c))f[c]=pb(c)/g(c)[3]}if(G){if(O!==1&&(O<1||i()[0]!==ea()))qa(i(),w([O,0,0,1,-O*Ea+ab,0],i()))}else if(t===-1){if(O!==1&&(O<1||i()[0]!==ea()))qa(i(),w([O,0,0,1,-O*Ea+ab,0],
i()));for(c=0;c<M();++c)if(f[c]!==1&&(f[c]<1||g(c)[3]!==T(c)))qa(g(c),w([1,0,0,f[c],0,-f[c]*sa+Fa],g(c)))}else if(f[t]!==1&&(f[t]<1||g(t)[3]!==T(t)))qa(g(t),w([1,0,0,f[t],0,-f[t]*sa+Fa],g(t)));ha();d=Ua(d,Q());C[v]=d[v];C[u]=d[u];r=e;Ya();Y();Ja()}$a=null},1))}};this.setXRange=function(a,b,c,d){b=o(0)[0]+o(0)[2]*b;c=o(0)[0]+o(0)[2]*c;if(q(o(0))>s(o(0))){if(b>q(o(0)))b=q(o(0));if(c<s(o(0)))c=s(o(0))}else{if(b<q(o(0)))b=q(o(0));if(c>s(o(0)))c=s(o(0))}var e=Pa(a);e=Kb(e,P(a),b,c,B(),h(),o(P(a)),l.minZoom,
l.maxZoom);b=e.xZoom;c=e.yZoom;e=e.panPoint;var f=ma(C,Q());i()[0]=b;if(c&&d)g(P(a))[3]=c;i()[4]=-e[v]*b;if(c&&d)g(P(a))[5]=-e[u]*c;Ha();a=Ua(f,Q());C[v]=a[v];C[u]=a[u];ha();Ya();Y();Ja()};this.getSeries=function(a){return Pa(a)};this.rangeChangedCallbacks=[];this.updateConfig=function(a){for(var b in a)if(a.hasOwnProperty(b))l[b]=a[b];Fb();Ya();Y();Ja()};this.updateConfig({});if(window.TouchEvent&&!window.MSPointerEvent&&!window.PointerEvent){ia.touchStart=aa.start;ia.touchEnd=aa.end;ia.touchMoved=
aa.moved}else{H=function(){};ia.touchStart=H;ia.touchEnd=H;ia.touchMoved=H}});
