...
b;break;case 2:d.scrollTop=b-(e-C());break;case 3:d.scrollTop=b-(e-C())/2;break}d.onscroll()}};var r=null;g.handleDragDrop=function(a,b,c,e,k){if(r){r.className=r.classNameOrig;r=null}if(a!="end"){var h=w(c);if(!h.selected&&h.drop)if(a=="drop")o.emit(g,{name:"dropEvent",eventObject:b,event:c},h.rowIdx,h.columnId,e,k);else{b.className="Wt-valid-drop";r=h.el;r.classNameOrig=r.className;r.className+=" Wt-drop-site"}else b.className=""}};g.onkeydown=function(a){var b=a||window.event;if(b.keyCode==9){f.cancelEvent(b);
var c=w(b);if(c.el){a=c.el.parentNode;c=x(c.el);var e=x(a),k=a.parentNode.childNodes.length,h=a.childNodes.length;b=b.shiftKey;for(var i=false,j=c,l;;){for(;b?j>=0:j<h;j=b?j-1:j+1)for(l=j==c&&!i?b?e-1:e+1:b?k-1:0;b?l>=0:l<k;l=b?l-1:l+1){if(j==c&&l==e)return;a=a.parentNode.childNodes[l];var n=$(a.childNodes[j]).find(":input");if(n.size()>0){setTimeout(function(){n.focus()},0);return}}j=b?h-1:0;i=true}}}else if(b.keyCode>=37&&b.keyCode<=40){i=f.target(b);function m(s){return f.hasTag(s,"INPUT")&&s.type==
"text"||f.hasTag(s,"TEXTAREA")}if(!f.hasTag(i,"SELECT")){c=w(b);if(c.el){a=c.el.parentNode;c=x(c.el);e=x(a);k=a.parentNode.childNodes.length;h=a.childNodes.length;switch(b.keyCode){case 39:if(m(i)){j=f.getSelectionRange(i);if(j.start!=i.value.length)return}e++;break;case 38:c--;break;case 37:if(m(i)){j=f.getSelectionRange(i);if(j.start!=0)return}e--;break;case 40:c++;break;default:return}f.cancelEvent(b);if(c>-1&&c<h&&e>-1&&e<k){a=a.parentNode.childNodes[e];n=$(a.childNodes[c]).find(":input");n.size()>
0&&setTimeout(function(){n.focus()},0)}}}}};this.autoJavaScript=function(){if(g.parentNode==null){g=d=p=null;this.autoJavaScript=function(){}}else if(!f.isHidden(g)){if(!f.isIE&&(u!=d.scrollTop||v!=d.scrollLeft)){p.scrollLeft=d.scrollLeft=v;q.scrollTop=d.scrollTop=u}var a=g.offsetWidth-f.px(g,"borderLeftWidth")-f.px(g,"borderRightWidth"),b=d.offsetWidth-d.clientWidth;a-=b;a-=q.clientWidth;if(a>200&&a!=d.tw){d.tw=a;d.style.width=a+b+"px";p.style.width=a+"px";if(!f.isIE)p.style.marginRight=b+"px"}a=
d.offsetHeight-d.clientHeight;if((b=q.style)&&b.marginBottom!==a+"px"){b.marginBottom=a+"px";o.layouts2.adjust(g.childNodes[0].id,[[1,0]])}}}});
