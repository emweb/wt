...
b.ondragleave=function(a){var c=a.clientX;a=a.clientY;var g=document.elementFromPoint(c,a);if(c===0&&a===0)g=null;if(g===i){e.setWidgetHoverStyle(false);m=1}else e.resetDragDrop()};b.ondragover=function(a){a.preventDefault()};bodyDragEnter=function(){if((o||p)&&$(b).is(":visible")){m=1;e.setPageHoverStyle()}};document.body.addEventListener("dragenter",bodyDragEnter);i.ondragover=function(a){a.preventDefault();a.stopPropagation()};i.ondragleave=function(){!l||m!=1||e.resetDragDrop()};i.ondrop=function(a){a.preventDefault();
p?b.ondrop(a):e.resetDragDrop()};b.ondrop=function(a){a.preventDefault();if(l){e.resetDragDrop();window.FormData===undefined||a.dataTransfer.files===null||a.dataTransfer.files.length===0||e.addFiles(a.dataTransfer.files)}};this.addFiles=function(a){for(var c=[],g=0;g<a.length;g++){var f={};f.id=Math.floor(Math.random()*Math.pow(2,31));f.file=a[g];d.push(f);var n={};n.id=f.id;n.filename=f.file.name;n.type=f.file.type;n.size=f.file.size;c.push(n)}k.emit(b,"dropsignal",JSON.stringify(c))};b.addEventListener("click",
