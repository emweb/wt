...
if(!j){window.removeEventListener("resize",i,{capture:true});window.removeEventListener("scroll",i,{capture:true})}}else{clearInterval(g);g=null}}var m=h.WT,k=0,b={},j=false,g=null;if(window.hasOwnProperty("MutationObserver"))g=new MutationObserver(i);this.add=function(a){k===0&&t();var c=a.el.id,e=c in b;j&&e&&b[c].observer&&b[c].observer.disconnect();var d=n(a.el,a.margin);if(a.visible!==d){a.visible=d;h.emit(a.el,"scrollVisibilityChanged",d)}b[c]=a;if(j){d=new IntersectionObserver(s,{rootMargin:""+
a.margin+"px"});d.observe(a.el);b[c].observer=d}e||++k};this.remove=function(a){if(k!==0){if(a in b){j&&b[a].observer&&b[a].observer.disconnect();delete b[a];--k}k===0&&u()}}});
