WT_DECLARE_WT_MEMBER(1,JavaScriptConstructor,"SizeHandle",function(c,k,e,r,v,w,x,y,h,f,l,n,o){function s(b){b=c.pageCoordinates(b);return Math.min(Math.max(k=="h"?b.x-g.x-d.x:b.y-g.y-d.y,v),w)}function i(b){var t=s(b);if(k=="h")a.style.left=d.x+t+"px";else a.style.top=d.y+t+"px";c.cancelEvent(b)}function u(b){if(a.parentNode!=null){a.parentNode.removeChild(a);y(s(b))}}var a=document.createElement("div");a.style.position="absolute";a.style.zIndex="100";if(k=="v"){a.style.width=r+"px";a.style.height=
...
p||(p="all");j.style["pointer-events"]="none";var q=document.body.style.cursor;q||(q="auto");document.body.style.cursor=k=="h"?"ew-resize":"ns-resize";function m(b){j.style["pointer-events"]=p;document.body.style.cursor=q;document.removeEventListener("mousemove",i);document.removeEventListener("mouseup",m);document.removeEventListener("touchmove",i);document.removeEventListener("touchend",m);u(b)}document.addEventListener("mousemove",i,{capture:true});document.addEventListener("mouseup",m,{capture:true});
document.addEventListener("touchmove",i,{capture:true});document.addEventListener("touchend",m,{capture:true})}else{a.onmousemove=f.ontouchmove=i;a.onmouseup=f.ontouchend=function(b){f.ontouchmove=null;f.ontouchend=null;u(b)}}});
