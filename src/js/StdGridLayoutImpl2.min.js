...
f.fs[a]=m>Math.max(T(f.w,a),u)?m+G(f.w,a):0}m=f.fs[a];if(f.layout){if(m==0)m=f.ps[a]}else{if(f.wasLayout){f.wasLayout=false;f.set=[false,false];y(f.w,s[0].size,"");y(f.w,s[1].size,"")}u=v(f.w,a);var V=typeof f.ps[a]!=="undefined"&&b.config[d][0]>0&&f.set[a];m=f.set[a]&&u>=f.psize[a]-4&&u<=f.psize[a]+4||V?Math.max(m,f.ps[a]):Math.max(m,u)}f.ps[a]=m;if(!f.span||f.span[a]==1)if(m>l)l=m}else if(!f.span||f.span[a]==1){if(f.ps[a]>l)l=f.ps[a];if(f.ms[a]>q)q=f.ms[a]}if(f.w.style.display!=="none"||e.hasTag(f.w,
