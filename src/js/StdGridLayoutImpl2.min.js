...
0,u=g.config[h][2],o=true;for(H=0;H<l;++H){var f=g.getItem(h,H);if(f){if(!f.w||a==0&&f.dirty>1){var n=$("#"+f.id),w=n.get(0);if(!w){g.setItem(h,H,null);continue}if(w!=f.w){f.w=w;n.find("img").add(n.filter("img")).bind("load",{item:f},function(S){Y(S.data.item,1,true)})}}if(!X&&f.w.style.position!="absolute"){f.w.style.position="absolute";f.w.style.visibility="hidden"}if(!f.ps)f.ps=[];if(!f.sc)f.sc=[];if(!f.ms)f.ms=[];if(!f.size)f.size=[];if(!f.psize)f.psize=[];if(!f.fs)f.fs=[];if(!f.margin)f.margin=
[];if(ka(f.w))f.ps[a]=f.ms[a]=0;else{n=!f.set;if(!f.set)f.set=[false,false];if(f.w){if(e.isIE)f.w.style.visibility="";if(f.dirty){if(f.dirty>1){w=C(f.w,a);f.ms[a]=w}else w=f.ms[a];if(w>u)u=w;if(f.dirty>1)f.margin[a]=x(f.w,a);if(!f.set[a])if(a==0||!n){n=e.pxself(f.w,g.size);f.fs[a]=n?n+f.margin[a]:0}else{n=Math.round(e.px(f.w,g.size));f.fs[a]=n>Math.max(G(f.w,a),w)?n+f.margin[a]:0}n=f.fs[a];if(f.layout){if(n==0)n=f.ps[a];f.ps[a]=n}else{if(f.wasLayout){f.wasLayout=false;f.set=[false,false];f.ps=[];
f.w.wtResize&&f.w.wtResize(f.w,-1,-1,true);v(f.w,s[1].size,"")}w=z(f.w,a,false,b);var K=w[0],T=f.set[a];if(T)if(f.psize[a]>8)T=K>=f.psize[a]-4&&K<=f.psize[a]+4;var la=typeof f.ps[a]!=="undefined"&&g.config[h][0]>0&&f.set[a];n=T||la?Math.max(n,f.ps[a]):Math.max(n,K);f.ps[a]=n;f.sc[a]=w[1]}if(!f.span||f.span[a]==1){if(n>p)p=n}else r=true}else if(!f.span||f.span[a]==1){if(f.ps[a]>p)p=f.ps[a];if(f.ms[a]>u)u=f.ms[a]}else r=true;if(!ka(f.w)&&(!f.span||f.span[a]==1))o=false}}}}if(o)u=p=-1;else if(u>p)p=
u;t[h]=p;d[h]=u;if(u>-1){j+=p;m+=u}}if(r){function pa(S,fa){for(h=0;h<i;++h)for(H=0;H<l;++H){var Z=g.getItem(h,H);if(Z&&Z.span&&Z.span[a]>1){var ca=S(Z),ga=0,ha=0,I;for(I=0;I<Z.span[a];++I){var Q=fa[h+I];if(Q!=-1){ca-=Q;++ga;if(g.config[h+I][0]>0)ha+=g.config[h+I][0]}}if(ca>=0)if(ga>0){if(ha>0)ga=ha;for(I=0;I<Z.span[a];++I){Q=fa[h+I];if(Q!=-1){Q=ha>0?g.config[h+I][0]:1;if(Q>0){var qa=Math.round(ca/Q);ca-=qa;ga-=Q;fa[h+I]+=qa}}}}else fa[h]=ca}}}pa(function(S){return S.ps[a]},t);pa(function(S){return S.ms[a]},
d)}b=0;n=true;r=false;for(h=0;h<i;++h)if(d[h]>-1){if(n){b+=g.margins[1];n=false}else{b+=g.margins[0];if(r)b+=4}r=g.config[h][1]!==0}n||(b+=g.margins[2]);j+=b;m+=b;g.measures=[t,d,j,m,b]}if(da||k[2]!=g.measures[2])ia.updateSizeInParent(a);c&&g.minSize==0&&k[3]!=g.measures[3]&&c.parentNode.className!="Wt-domRoot"&&v(c,"min"+g.Size,g.measures[3]+"px");c&&a==0&&c&&e.hasTag(c,"TD")&&v(c,g.size,g.measures[2]+"px")}function ta(a,b,c){a=s[a];if(ea)c=-c;if(a.config[b][0]>0&&a.config[b+1][0]==0){++b;c=-c}a.fixedSize[b]=
...
false,j=ja?b.parentNode:null;if(c.maxSize===0)if(j){var m=e.css(j,"position");if(m==="absolute")l=e.pxself(j,c.size);if(l===0){if(!c.initialized){if(a===0&&(m==="absolute"||m==="fixed")){j.style.display="none";l=j.clientWidth;j.style.display=""}l=a?j.clientHeight:j.clientWidth;k=true;if(a==0&&l==0&&e.isIElt9){l=j.offsetWidth;k=false}var h;if((e.hasTag(j,"TD")||e.hasTag(j,"TH"))&&!(e.isIE&&!e.isIElt9)){d=0;h=1}else{d=c.minSize?c.minSize:i[3];h=0}function H(T,la){return T-la<=1}if(e.isIElt9&&H(l,h)||
H(l,d+N(j,a)))c.maxSize=999999}if(l===0&&c.maxSize===0){l=a?j.clientHeight:j.clientWidth;k=true}}}else{l=e.pxself(b,c.size);t=true}else if(c.sizeSet){l=e.pxself(j,c.size);t=true}var r=0;if(j&&j.wtGetPS&&a==1)r=j.wtGetPS(j,b,a,0);d=i[2];if(d<c.minSize)d=c.minSize;if(c.maxSize&&!c.sizeSet){l=Math.min(d,c.maxSize)+r;v(j,c.size,l+G(j,a)+"px")&&O&&O.setElDirty(J);l=l;d=t=true}c.cSize=l;if(a==1&&j&&j.wtResize){h=g.cSize;d=c.cSize;j.wtResize(j,Math.round(h),Math.round(d),true)}l-=r;if(!t){t=0;if(typeof c.cPadding===
"undefined"){t=k?N(j,a):G(j,a);c.cPadding=t}else t=c.cPadding;l-=t}c.initialized=true;if(!(j&&l<=0)){if(l<i[3]-r)l=i[3]-r;j=[];k=c.config.length;t=g.config.length;for(d=0;d<k;++d)c.stretched[d]=false;if(l>=i[3]-r){r=l-i[4];h=[];var p=[0,0],u=[0,0],o=0;for(d=0;d<k;++d)if(i[1][d]>-1){m=-1;if(typeof c.fixedSize[d]!=="undefined"&&!va(c.config,d))c.fixedSize[d]=undefined;if(typeof c.fixedSize[d]!=="undefined"&&(d+1==k||i[1][d+1]>-1))m=c.fixedSize[d];else if(c.config[d][1]!==0&&c.config[d][1][0]>=0){m=
