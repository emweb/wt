...
if(a==1&&m&&m.wtResize){i=g.cSize;m.wtResize(m,i,c.cSize)}j-=d;if(!w){w=0;if(c.initialized)w=c.cPadding;else{w=o?Y(m,a):S(m,a);c.cPadding=w}j-=w}c.initialized=true;if(!(m&&j<=0)){if(j<h[3]-d)j=h[3]-d;$(b).children("."+g.handleClass).css(c.size,j-c.margins[2]-c.margins[1]+"px");o=[];m=c.config.length;w=g.config.length;if(j>=h[3]-d){var p=j-h[4];i=[];var l=[0,0],q=[0,0],n=0;for(d=0;d<m;++d)if(h[1][d]>-1){k=-1;if(typeof c.fixedSize[d]!=="undefined")k=c.fixedSize[d];else if(c.config[d][1]!==0&&c.config[d][1][0]>=
0){k=c.config[d][1][0];if(c.config[d][1][1])k=(j-h[4])*k/100}if(k>=0){i[d]=-1;o[d]=k;p-=o[d]}else{if(c.config[d][0]>0){k=1;i[d]=c.config[d][0];n+=i[d]}else{k=0;i[d]=0}l[k]+=h[1][d];q[k]+=h[0][d];o[d]=h[0][d]}}else i[d]=-2;if(n==0){for(d=0;d<m;++d)if(i[d]==0){i[d]=1;++n}q[1]=q[0];l[1]=l[0];q[0]=0;l[0]=0}if(p>q[0]+l[1]){p-=q[0];if(p>q[1]){if(c.fitSize){p-=q[1];j=p/n;for(d=0;d<m;++d)if(i[d]>0)o[d]+=Math.round(i[d]*j)}}else{k=1;if(p<l[k])p=l[k];j=q[k]-l[k]>0?(p-l[k])/(q[k]-l[k]):0;for(d=0;d<m;++d)if(i[d]>
0){p=h[0][d]-h[1][d];o[d]=h[1][d]+Math.round(p*j)}}}else{for(d=0;d<m;++d)if(i[d]>0)o[d]=h[1][d];p-=l[1];k=0;if(p<l[k])p=l[k];j=q[k]-l[k]>0?(p-l[k])/(q[k]-l[k]):0;for(d=0;d<m;++d)if(i[d]==0){p=h[0][d]-h[1][d];o[d]=h[1][d]+Math.round(p*j)}}}else o=h[1];c.sizes=o;h=0;j=true;l=false;for(d=0;d<m;++d)if(o[d]>-1){if(p=l){i=A+"-rs"+a+"-"+d;l=e.getElement(i);if(!l){l=document.createElement("div");l.setAttribute("id",i);l.di=d;l.style.position="absolute";l.style[g.left]=g.margins[1]+"px";l.style[c.size]=c.margins[0]+
"px";l.className=c.handleClass;b.insertBefore(l,b.firstChild);l.onmousedown=l.ontouchstart=function(F){ca(a,this,F||window.event)}}h+=2;y(l,c.left,h+"px");h+=2}l=c.config[d][1]!==0;if(j){h+=c.margins[1];j=false}else h+=c.margins[0];for(q=0;q<w;++q)if((n=c.getItem(d,q))&&n.w){i=n.w;k=o[d];if(n.span){var f,x=l;for(f=1;f<n.span[a];++f){if(x)k+=4;x=c.config[d+x][1]!==0;k+=c.margins[0];k+=o[d+f]}}var r;i.style.visibility="";x=n.align>>c.alignBits&15;f=n.ps[a];if(k<f)x=0;if(x){switch(x){case 1:r=h;break;
case 4:r=h+(k-f)/2;break;case 2:r=h+(k-f);break}if(n.layout){y(i,c.size,f+"px")&&P(n);n.set[a]=true}else if(k>=f&&n.set[a]){y(i,c.size,f+"px")&&P(n);n.set[a]=false}n.size[a]=f;n.psize[a]=f}else{r=E(n.w,a);x=k;if(e.isIElt9||!e.hasTag(i,"BUTTON")&&!e.hasTag(i,"INPUT")&&!e.hasTag(i,"SELECT")&&!e.hasTag(i,"TEXTAREA"))x=Math.max(0,x-r);r=false;if(e.isIE&&e.hasTag(i,"BUTTON"))r=true;if(r||k!=f||n.layout){y(i,c.size,x+"px")&&P(n);n.set[a]=true}else if(n.fs[a])a==0&&y(i,c.size,n.fs[a]+"px");else{y(i,c.size,
"")&&P(n);n.set[a]=false}r=h;n.size[a]=x;n.psize[a]=k}if(t)if(p){y(i,c.left,"4px");k=e.css(i,"position");if(k!=="absolute")i.style.position="relative"}else y(i,c.left,"0px");else y(i,c.left,r+"px");if(a==1){i.wtResize&&i.wtResize(i,n.size[0],n.size[1]);n.dirty=false}}h+=o[d]}}}var e=G.WT;this.ancestor=null;this.descendants=[];var V=this,C=z,L=false,M=true,X=false,U=null,Q=null,Z=false,J=[],W=0,R=$(document.body).hasClass("Wt-rtl"),s=[{initialized:false,config:C.cols,margins:B,maxSize:H,measures:[],
sizes:[],fixedSize:[],Left:R?"Right":"Left",left:R?"right":"left",Right:R?"Left":"Right",Size:"Width",size:"width",alignBits:0,getItem:function(a,b){return C.items[b*s[0].config.length+a]},handleClass:"Wt-vrh2",resizeDir:"h",resizerClass:"Wt-hsh2",fitSize:N},{initialized:false,config:C.rows,margins:v,maxSize:D,measures:[],sizes:[],fixedSize:[],Left:"Top",left:"top",Right:"Bottom",Size:"Height",size:"height",alignBits:4,getItem:function(a,b){return C.items[a*s[0].config.length+b]},handleClass:"Wt-hrh2",
resizeDir:"v",resizerClass:"Wt-vsh2",fitSize:O}];jQuery.data(document.getElementById(A),"layout",this);this.setConfig=function(a){var b=C;C=a;s[0].config=C.cols;s[1].config=C.rows;var c;a=0;for(c=b.items.length;a<c;++a){var g=b.items[a];if(g){g.set[0]&&y(g.w,s[0].size,"");g.set[1]&&y(g.w,s[1].size,"");if(g.layout){V.setChildSize(g.w,0,g.ps[0]);V.setChildSize(g.w,1,g.ps[1])}}}M=true};this.getId=function(){return A};this.setItemsDirty=function(a){var b,c,g=s[0].config.length;b=0;for(c=a.length;b<c;++b)C.items[a[b][0]*
g+a[b][1]].dirty=true;L=true};this.setDirty=function(){M=true};this.setChildSize=function(a,b,c){var g,h;g=0;for(h=C.items.length;g<h;++g){var j=C.items[g];if(j&&j.id==a.id){if(!j.ps)j.ps=[];j.ps[b]=c;j.layout=true;break}}L=true};this.measure=function(a){var b=e.getElement(A);if(b)if(!e.isHidden(b)){if(!Z){Z=true;if(X=K==null){var c=b,g=c.parentNode;for(J=[0,0];;){J[0]+=T(g,0);J[1]+=T(g,1);if(g.childNodes.length>1){var h,j;h=0;for(j=g.childNodes.length;h<j;++h)if(g.childNodes[h]!=c){J[1]+=g.childNodes[h].offsetHeight;
W=Math.max(g.childNodes[h].offsetWidth,W)}}if(c=jQuery.data(g.parentNode,"layout")){U=c;Q=g;break}c=g;g=c.parentNode;if(g.childNodes.length!=1)break}}else{U=jQuery.data(document.getElementById(K),"layout");Q=b;J[0]=T(Q,0);J[1]=T(Q,1)}}if(L||M)aa(a,b,X?b.parentNode:null);if(a==1)L=M=false}};this.apply=function(a){var b=e.getElement(A);if(!b)return false;if(e.isHidden(b))return true;da(a,b);return true};this.contains=function(a){var b=e.getElement(A);a=e.getElement(a.getId());return b&&a?e.contains(b,
a):false}});WT_DECLARE_WT_MEMBER(2,JavaScriptPrototype,"StdLayout2.prototype.initResize",function(){this.resizeInitialized=true});
