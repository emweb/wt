...
g=1}else{d=i[3];g=0}if(e.isIElt9&&k==g||k==d+X(m,a))c.maxSize=999999}if(k===0&&c.maxSize===0){k=a?m.clientHeight:m.clientWidth;o=true}}}else{k=e.pxself(b,c.size);w=true}d=0;if(a==1&&e.hasTag(m,"FIELDSET")&&m.children.length==2)d=m.firstChild.offsetHeight;g=i[2];if(g<c.minSize)g=c.minSize;if(c.maxSize)if(g<c.maxSize){y(m,c.size,g+S(m,a)+"px");k=g;g=w=true}else{k=c.maxSize;o=false}c.cSize=k;if(a==1&&m&&m.wtResize){g=h.cSize;m.wtResize(m,g,c.cSize)}k-=d;if(!w){w=0;if(c.initialized)w=c.cPadding;else{w=
o?X(m,a):S(m,a);c.cPadding=w}k-=w}c.initialized=true;if(!(m&&k<=0)){if(k<i[3]-d)k=i[3]-d;$(b).children("."+h.handleClass).css(c.size,k-c.margins[2]-c.margins[1]+"px");o=[];m=c.config.length;w=h.config.length;if(k>=i[3]-d){var p=k-i[4];g=[];var l=[0,0],q=[0,0],n=0;for(d=0;d<m;++d)if(i[1][d]>-1){j=-1;if(typeof c.fixedSize[d]!=="undefined")j=c.fixedSize[d];else if(c.config[d][1]!==0&&c.config[d][1][0]>=0){j=c.config[d][1][0];if(c.config[d][1][1])j=(k-i[4])*j/100}if(j>=0){g[d]=-1;o[d]=j;p-=o[d]}else{if(c.config[d][0]>
0){j=1;g[d]=c.config[d][0];n+=g[d]}else{j=0;g[d]=0}l[j]+=i[1][d];q[j]+=i[0][d];o[d]=i[0][d]}}else g[d]=-2;if(n==0){for(d=0;d<m;++d)if(g[d]==0){g[d]=1;++n}q[1]=q[0];l[1]=l[0];q[0]=0;l[0]=0}if(p>q[0]+l[1]){p-=q[0];if(p>q[1]){if(c.fitSize){p-=q[1];k=p/n;for(d=0;d<m;++d)if(g[d]>0)o[d]+=Math.round(g[d]*k)}}else{j=1;if(p<l[j])p=l[j];k=q[j]-l[j]>0?(p-l[j])/(q[j]-l[j]):0;for(d=0;d<m;++d)if(g[d]>0){p=i[0][d]-i[1][d];o[d]=i[1][d]+Math.round(p*k)}}}else{for(d=0;d<m;++d)if(g[d]>0)o[d]=i[1][d];p-=l[1];j=0;if(p<
l[j])p=l[j];k=q[j]-l[j]>0?(p-l[j])/(q[j]-l[j]):0;for(d=0;d<m;++d)if(g[d]==0){p=i[0][d]-i[1][d];o[d]=i[1][d]+Math.round(p*k)}}}else o=i[1];c.sizes=o;i=0;k=true;l=false;for(d=0;d<m;++d)if(o[d]>-1){if(p=l){g=A+"-rs"+a+"-"+d;l=e.getElement(g);if(!l){l=document.createElement("div");l.setAttribute("id",g);l.di=d;l.style.position="absolute";l.style[h.left]=h.margins[1]+"px";l.style[c.size]=c.margins[0]+"px";l.className=c.handleClass;b.insertBefore(l,b.firstChild);l.onmousedown=l.ontouchstart=function(F){ba(a,
this,F||window.event)}}i+=2;y(l,c.left,i+"px");i+=2}l=c.config[d][1]!==0;if(k){i+=c.margins[1];k=false}else i+=c.margins[0];for(q=0;q<w;++q)if((n=c.getItem(d,q))&&n.w){g=n.w;j=o[d];if(n.span){var f,x=l;for(f=1;f<n.span[a];++f){if(x)j+=4;x=c.config[d+x][1]!==0;j+=c.margins[0];j+=o[d+f]}}var r;g.style.visibility="";x=n.align>>c.alignBits&15;f=n.ps[a];if(j<f)x=0;if(x){switch(x){case 1:r=i;break;case 4:r=i+(j-f)/2;break;case 2:r=i+(j-f);break}if(n.layout){y(g,c.size,f+"px")&&P(n);n.set[a]=true}else if(j>=
f&&n.set[a]){y(g,c.size,f+"px")&&P(n);n.set[a]=false}n.size[a]=f;n.psize[a]=f}else{r=E(n.w,a);x=j;if(e.isIElt9||!e.hasTag(g,"BUTTON")&&!e.hasTag(g,"INPUT")&&!e.hasTag(g,"SELECT")&&!e.hasTag(g,"TEXTAREA"))x=Math.max(0,x-r);r=false;if(e.isIE&&e.hasTag(g,"BUTTON"))r=true;if(r||j!=f||n.layout){y(g,c.size,x+"px")&&P(n);n.set[a]=true}else if(n.fs[a])a==0&&y(g,c.size,n.fs[a]+"px");else{y(g,c.size,"")&&P(n);n.set[a]=false}r=i;n.size[a]=x;n.psize[a]=j}if(t)if(p){y(g,c.left,"4px");j=e.css(g,"position");if(j!==
"absolute")g.style.position="relative"}else y(g,c.left,"0px");else y(g,c.left,r+"px");if(a==1){g.wtResize&&g.wtResize(g,n.size[0],n.size[1]);n.dirty=false}}i+=o[d]}}}var e=G.WT;this.ancestor=null;this.descendants=[];var V=this,C=z,J=false,K=true,W=false,U=null,Q=null,Y=false,L=[],R=$(document.body).hasClass("Wt-rtl"),s=[{initialized:false,config:C.cols,margins:B,maxSize:H,measures:[],sizes:[],fixedSize:[],Left:R?"Right":"Left",left:R?"right":"left",Right:R?"Left":"Right",Size:"Width",size:"width",
alignBits:0,getItem:function(a,b){return C.items[b*s[0].config.length+a]},handleClass:"Wt-vrh2",resizeDir:"h",resizerClass:"Wt-hsh2",fitSize:N},{initialized:false,config:C.rows,margins:v,maxSize:D,measures:[],sizes:[],fixedSize:[],Left:"Top",left:"top",Right:"Bottom",Size:"Height",size:"height",alignBits:4,getItem:function(a,b){return C.items[a*s[0].config.length+b]},handleClass:"Wt-hrh2",resizeDir:"v",resizerClass:"Wt-vsh2",fitSize:O}];jQuery.data(document.getElementById(A),"layout",this);this.setConfig=
function(a){var b=C;C=a;s[0].config=C.cols;s[1].config=C.rows;var c;a=0;for(c=b.items.length;a<c;++a){var h=b.items[a];if(h){h.set[0]&&y(h.w,s[0].size,"");h.set[1]&&y(h.w,s[1].size,"");if(h.layout){V.setChildSize(h.w,0,h.ps[0]);V.setChildSize(h.w,1,h.ps[1])}}}K=true};this.getId=function(){return A};this.setItemsDirty=function(a){var b,c,h=s[0].config.length;b=0;for(c=a.length;b<c;++b)C.items[a[b][0]*h+a[b][1]].dirty=true;J=true};this.setDirty=function(){K=true};this.setChildSize=function(a,b,c){var h,
i;h=0;for(i=C.items.length;h<i;++h){var k=C.items[h];if(k&&k.id==a.id){if(!k.ps)k.ps=[];k.ps[b]=c;k.layout=true;break}}J=true};this.measure=function(a){var b=e.getElement(A);if(b)if(!e.isHidden(b)){if(!Y){Y=true;if(W=M==null){var c=b.parentNode;for(L=[0,0];;){L[0]+=T(c,0);L[1]+=T(c,1);var h=jQuery.data(c.parentNode,"layout");if(h){U=h;Q=c;break}c=c.parentNode;if(c.childNodes.length!=1)break}}else{U=jQuery.data(document.getElementById(M),"layout");Q=b;L[0]=T(Q,0);L[1]=T(Q,1)}}if(J||K)Z(a,b,W?b.parentNode:
