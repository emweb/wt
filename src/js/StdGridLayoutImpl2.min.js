...
j=e.measures.slice();if(j.length==5){j[0]=j[0].slice();j[1]=j[1].slice()}if(N){if(c&&typeof e.minSize=="undefined"){e.minSize=f.px(c,"min"+e.Size);if(e.minSize>0)e.minSize-=G(c,a)}var t=[],d=[],n=0,l=0,h,I,r=false;for(h=0;h<i;++h){var m=0,u=e.config[h][2],p=true;for(I=0;I<k;++I){var g=e.getItem(h,I);if(g){if(!g.w||a==0&&g.dirty>1){var o=$("#"+g.id),w=o.get(0);if(!w){e.setItem(h,I,null);continue}if(w!=g.w){g.w=w;o.find("img").add(o.filter("img")).bind("load",{item:g},function(P){W(P.data.item,1,true)})}}if(!V&&
...
g.ps[a]=o;g.sc[a]=w[1]}if(!g.span||g.span[a]==1){if(o>m)m=o}else r=true}else if(!g.span||g.span[a]==1){if(g.ps[a]>m)m=g.ps[a];if(g.ms[a]>u)u=g.ms[a]}else r=true;if(!(g.w.style.display==="none"&&!g.w.ed)&&(!g.span||g.span[a]==1))p=false}}}}if(p)u=m=-1;else if(u>m)m=u;t[h]=m;d[h]=u;if(u>-1){n+=m;l+=u}}if(r){function ma(P,fa){for(h=0;h<i;++h)for(I=0;I<k;++I){var X=e.getItem(h,I);if(X&&X.span&&X.span[a]>1){var ca=P(X),ga=0,ha=0,H;for(H=0;H<X.span[a];++H){var O=fa[h+H];if(O!=-1){ca-=O;++ga;if(e.config[h+
H][0]>0)ha+=e.config[h+H][0]}}if(ca>=0)if(ga>0){if(ha>0)ga=ha;for(H=0;H<X.span[a];++H){O=fa[h+H];if(O!=-1){O=ha>0?e.config[h+H][0]:1;if(O>0){var na=Math.round(ca/O);ca-=na;ga-=O;fa[h+H]+=na}}}}else fa[h]=ca}}}ma(function(P){return P.ps[a]},t);ma(function(P){return P.ms[a]},d)}b=0;o=true;r=false;for(h=0;h<i;++h)if(d[h]>-1){if(o){b+=e.margins[1];o=false}else{b+=e.margins[0];if(r)b+=4}r=e.config[h][1]!==0}o||(b+=e.margins[2]);n+=b;l+=b;e.measures=[t,d,n,l,b]}if(da||j[2]!=e.measures[2])ia.updateSizeInParent(a);
...
0&&f.isIElt9){k=n.offsetWidth;j=false}var h;if((f.hasTag(n,"TD")||f.hasTag(n,"TH"))&&!(f.isIE&&!f.isIElt9)){d=0;h=1}else{d=c.minSize?c.minSize:i[3];h=0}function I(Q,ka){return Q-ka<=1}if(f.isIElt9&&epsEqual(k,h)||I(k,d+M(n,a)))c.maxSize=999999}if(k===0&&c.maxSize===0){k=a?n.clientHeight:n.clientWidth;j=true}}}else{k=f.pxself(b,c.size);t=true}else if(c.sizeSet){k=f.pxself(n,c.size);t=true}var r=0;if(n&&n.wtGetPS&&a==1)r=n.wtGetPS(n,b,a,0);d=i[2];if(d<c.minSize)d=c.minSize;if(c.maxSize&&!c.sizeSet){k=
Math.min(d,c.maxSize)+r;v(n,c.size,k+G(n,a)+"px")&&Y&&Y.setElDirty(S);k=k;d=t=true}c.cSize=k;if(a==1&&n&&n.wtResize){h=e.cSize;d=c.cSize;n.wtResize(n,Math.round(h),Math.round(d),true)}k-=r;if(!t){t=0;if(typeof c.cPadding==="undefined"){t=j?M(n,a):G(n,a);c.cPadding=t}else t=c.cPadding;k-=t}c.initialized=true;if(!(n&&k<=0)){if(k<i[3]-r)k=i[3]-r;j=[];n=c.config.length;t=e.config.length;for(d=0;d<n;++d)c.stretched[d]=false;if(k>=i[3]-r){r=k-i[4];h=[];var m=[0,0],u=[0,0],p=0;for(d=0;d<n;++d)if(i[1][d]>
...
E+"-rs"+a+"-"+d;m=f.getElement(h);if(!m){m=document.createElement("div");m.setAttribute("id",h);m.di=d;m.style.position="absolute";m.style[e.left]=e.margins[1]+"px";m.style[c.size]=c.margins[0]+"px";if(e.cSize)m.style[e.size]=e.cSize-e.margins[2]-e.margins[1]+"px";m.className=c.handleClass;b.insertBefore(m,b.firstChild);m.onmousedown=m.ontouchstart=function(Q){sa(a,this,Q||window.event)}}i+=2;v(m,c.left,i+"px");i+=2}m=c.config[d][1]!==0;if(r)r=false;else i+=c.margins[0]}for(u=0;u<t;++u)if((p=c.getItem(d,
u))&&p.w){h=p.w;l=Math.max(j[d],0);if(p.span){var o,w=m;for(o=1;o<p.span[a];++o){if(d+o>=j.length)break;if(w)l+=4;w=c.config[d+o][1]!==0;if(j[d+o-1]>-1&&j[d+o]>-1)l+=c.margins[0];l+=j[d+o]}}var J;v(h,"visibility","");w=p.align>>c.alignBits&15;o=p.ps[a];if(l<o)w=0;if(w){switch(w){case 1:J=i;break;case 4:J=i+(l-o)/2;break;case 2:J=i+(l-o);break}o-=p.margin[a];if(p.layout){v(h,c.size,o+"px")&&W(p,1);p.set[a]=true}else if(l>=o&&p.set[a]){v(h,c.size,o+"px")&&W(p,1);p.set[a]=false}p.size[a]=o;p.psize[a]=
o}else{w=Math.max(0,l-p.margin[a]);J=a==0&&p.sc[a];if(!(h.style.display==="none"&&!h.ed)&&(J||l!=o||p.layout)){if(v(h,c.size,w+"px")){if(!f.isIE&&(f.hasTag(h,"TEXTAREA")||f.hasTag(h,"INPUT"))){v(h,"margin-"+c.left,p.margin[a]/2+"px");v(h,"margin-"+e.left,p.margin[!a]/2+"px")}W(p,1);p.set[a]=true}}else if(p.fs[a])a==0&&v(h,c.size,p.fs[a]+"px");else{v(h,c.size,"")&&W(p,1);p.set[a]=false}J=i;p.size[a]=w;p.psize[a]=l}if(V)if(g){v(h,c.left,"4px");l=f.css(h,"position");if(l!=="absolute")h.style.position=
"relative"}else v(h,c.left,"0px");else v(h,c.left,J+"px");if(a==1){if(h.wtResize)h.wtResize(h,p.set[0]?Math.round(p.size[0]):-1,p.set[1]?Math.round(p.size[1]):-1,true);p.dirty=0}}if(j[d]>-1)i+=j[d]}$(b).children("."+e.handleClass).css(c.size,k-c.margins[2]-c.margins[1]+"px")}}var f=F.WT;this.descendants=[];var ia=this,B=A,N=true,da=true,ja=false,Y=null,S=null,la=false,oa=false,Z=[],pa=false,ea=$(document.body).hasClass("Wt-rtl"),s=[{initialized:false,config:B.cols,margins:D,maxSize:aa,measures:[],
sizes:[],stretched:[],fixedSize:[],Left:ea?"Right":"Left",left:ea?"right":"left",Right:ea?"Left":"Right",Size:"Width",size:"width",alignBits:0,getItem:function(a,b){return B.items[b*s[0].config.length+a]},setItem:function(a,b,c){B.items[b*s[0].config.length+a]=c},handleClass:"Wt-vrh2",resizeDir:"h",resizerClass:"Wt-hsh2",fitSize:T},{initialized:false,config:B.rows,margins:y,maxSize:q,measures:[],sizes:[],stretched:[],fixedSize:[],Left:"Top",left:"top",Right:"Bottom",Size:"Height",size:"height",alignBits:4,
