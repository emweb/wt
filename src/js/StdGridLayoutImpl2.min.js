WT_DECLARE_WT_MEMBER(1,JavaScriptConstructor,"StdLayout2",function(F,A,L,M,N,r,G,D,B,y,w){function u(a,b){var c=q[b],j=b?a.scrollHeight:a.scrollWidth,g=b?a.clientHeight:a.clientWidth;b=b?a.offsetHeight:a.offsetWidth;if(g>=1E6){g-=1E6;if(j>=1E6)j-=1E6}if(j===0)j=f.pxself(a,c.size);if(f.isIE&&(f.hasTag(a,"BUTTON")||f.hasTag(a,"TEXTAREA")||f.hasTag(a,"INPUT")||f.hasTag(a,"SELECT")))j=g;if(!f.isOpera&&!f.isGecko)j+=f.px(a,"border"+c.Left+"Width")+f.px(a,"border"+c.Right+"Width");j+=f.px(a,"margin"+c.Left)+
...
f.px(a,"margin"+b.Left)+f.px(a,"margin"+b.Right)+f.px(a,"padding"+b.Left)+f.px(a,"padding"+b.Right)}function O(a,b){I=a.dirty=true;b&&F.layouts2.scheduleAdjust()}function z(a,b,c){if(a.style[b]!==c){a.style[b]=c;return true}else return false}function X(a,b,c){b=q[a];var j=b.config.length,g=q[a^1].config.length;if(I||J){var k=b.measures.slice();if(k.length==5){k[0]=k[0].slice();k[1]=k[1].slice()}var p=[],x=[],e=0,m=0,h,i;for(h=0;h<j;++h){var n=0,l=b.config[h][2],s=true;for(i=0;i<g;++i){var d=b.getItem(h,
...
true;setTimeout(function(){L.adjust()},0)}};this.adjust=function(r,G){function D(y,w){var u,H;u=0;for(H=y.length;u<H;++u){var E=y[u];D(E.descendants,w);w==1&&M&&E.setDirty();E.measure(w)}}function B(y,w){var u,H;u=0;for(H=y.length;u<H;++u){var E=y[u];if(E.apply(w))B(E.descendants,w);else{y.splice(u,1);--u;--H}}}if(r)(r=this.find(r))&&r.setItemsDirty(G);else{N=false;if(!A){A=true;D(F,0);B(F,0);D(F,1);B(F,1);M=A=false}}};this.updateConfig=function(r,G){(r=this.find(r))&&r.setConfig(G)};window.onresize=
