...
WT_DECLARE_WT_MEMBER(2,"WSuggestionPopupStdMatcher",function(v,e,y,A,t,o){function w(c){var f=c.value;c=c.selectionStart?c.selectionStart:f.length;for(var j=y?f.lastIndexOf(y,c-1)+1:0;j<c&&A.indexOf(f.charAt(j))!=-1;)++j;return{start:j,end:c}}this.match=function(c){var f=w(c),j=c.value.substring(f.start,f.end),m="^";if(t.length!=0)m="(^|(?:["+t+"]))";m+="("+j.replace(/([\^\\\][\-.$*+?()|{}])/g,"\\$1")+")";m=new RegExp(m,"gi");return function(p){if(!p)return j;var x=false;if(j.length){var q=p.replace(m,
"$1"+v+"$2"+e);if(q!=p){x=true;p=q}}return{match:x,suggestion:p}}};this.replace=function(c,f,j){f=w(c);var m=c.value.substring(0,f.start)+j+o;if(f.end<c.value.length)m+=c.value.substring(f.end,c.value.length);c.value=m;if(c.selectionStart){c.selectionStart=f.start+j.length+o.length;c.selectionEnd=c.selectionStart}}});
