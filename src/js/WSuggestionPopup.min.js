...
F=false,G=null,H=null,B=null,D=null,s=false;this.defaultValue=y;this.showPopup=function(){f.style.display="";D=n=null};this.editMouseMove=function(a,b){if(p(a))a.style.cursor=e.widgetCoordinates(a,b).x>a.offsetWidth-16?"default":""};this.editClick=function(a,b){if(p(a))if(e.widgetCoordinates(a,b).x>a.offsetWidth-16)if(h!=a.id){d();h=a.id;s=true;A.refilter()}else{h=null;d()}};this.editKeyDown=function(a,b){if(!p(a))return true;if(h!=a.id)if($(a).hasClass("Wt-suggest-onedit")){h=a.id;s=false}else if($(a).hasClass("Wt-suggest-dropdown")&&
