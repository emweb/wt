...
b.keyCode==34||b.keyCode==33){if(b.type.toUpperCase()=="KEYDOWN"){J=true;d.cancelEvent(b,d.CancelDefaultAction)}if(b.type.toUpperCase()=="KEYPRESS"&&J==true){d.cancelEvent(b);return false}var m=l,p=b.keyCode==40||b.keyCode==34;b=b.keyCode==34||b.keyCode==33?e.clientHeight/l.offsetHeight:1;var j;for(j=0;m&&j<b;++j){var x=C(m,p);if(!x)break;m=x}if(m&&d.hasTag(m,"LI")){l.className="";m.className="active";o=m.id}return false}return b.keyCode!=13&&b.keyCode!=9};this.filtered=function(a,b){D=a;M=b;s.refilter()};
this.refilter=function(){var a=o?d.getElement(o):null,b=d.getElement(f),l=F(b),m=e.childNodes,p=l(null);G=b.value;if(r>0||u)if(p.length<r&&!w){i();return}else{var j=M?p:p.substring(0,Math.max(D!==null?D.length:0,r));if(j!=D)if(j!=K){K=j;t.emit(e,"filter",j)}}var x=j=null;p=w&&p.length==0;var y,N;y=0;for(N=m.length;y<N;++y){var k=m[y];if(d.hasTag(k,"LI")){var H=k.firstChild;if(k.orig==null)k.orig=H.firstChild.innerHTML;var I=l(k.orig),O=p||I.match;if(I.suggestion!=H.firstChild.innerHTML)H.firstChild.innerHTML=
I.suggestion;if(O){if(k.style.display!="")k.style.display="";if(j==null)j=k;if(y==this.defaultValue)x=k}else if(k.style.display!="none")k.style.display="none";if(k.className!="")k.className=""}}if(j==null)i();else{if(!c()){g(b);s.showPopup(b);a=null}if(!a||a.style.display=="none"){a=x||j;a.parentNode.scrollTop=0;o=a.id}a.className="active";v(a)}};this.editKeyUp=function(a,b){if(f!=null)if(q(a))if(!(!c()&&(b.keyCode==13||b.keyCode==9)))if(b.keyCode==27||b.keyCode==37||b.keyCode==39)i();else if(a.value!=
