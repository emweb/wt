...
function(a,b){if(c(a))if(d.widgetCoordinates(a,b).x>a.offsetWidth-16)if(f!=a.id||!e())r.showAt(a,"");else{m();f=null}};this.editKeyDown=function(a,b){if(!c(a))return true;if(f!=a.id)if($(a).hasClass("Wt-suggest-onedit")){f=a.id;v=false}else if($(a).hasClass("Wt-suggest-dropdown")&&b.keyCode==40){f=a.id;v=true}else{f=null;return true}var h=o?d.getElement(o):null;if(e()&&h)if(b.keyCode==13||b.keyCode==9){p(h);d.cancelEvent(b);setTimeout(function(){a.focus()},0);return false}else if(b.keyCode==40||b.keyCode==
38||b.keyCode==34||b.keyCode==33){if(b.type.toUpperCase()=="KEYDOWN"){K=true;d.cancelEvent(b,d.CancelDefaultAction)}if(b.type.toUpperCase()=="KEYPRESS"&&K==true){d.cancelEvent(b);return false}var k=h,F=b.keyCode==40||b.keyCode==34;b=b.keyCode==34||b.keyCode==33?g.clientHeight/h.offsetHeight:1;var n;for(n=0;k&&n<b;++n){var w=B(k,F);if(!w)break;k=w}if(k&&d.hasTag(k,"LI")){h.className="";k.className="active";o=k.id}return false}return b.keyCode!=13&&b.keyCode!=9};this.filtered=function(a,b){C=a;N=b;
r.refilter(E)};this.refilter=function(a){if(f){var b=o?d.getElement(o):null,h=d.getElement(f),k=G(h),F=g.childNodes,n=u&&a!=null?a:k(null);E=u?a:h.value;if(q>0||t)if(n.length<q&&!v){m();return}else{a=N?n:n.substring(0,Math.max(C!==null?C.length:0,q));if(a!=C)if(a!=M){M=a;s.emit(g,"filter",a)}}var w=a=null;n=v&&n.length==0;var x,O;x=0;for(O=F.length;x<O;++x){var j=F[x];if(d.hasTag(j,"LI")){var H=j.firstChild;if(j.orig==null)j.orig=H.firstChild.innerHTML;var I=k(j.orig),P=n||I.match;if(I.suggestion!=
H.firstChild.innerHTML)H.firstChild.innerHTML=I.suggestion;if(P){if(j.style.display!="")j.style.display="";if(a==null)a=j;if(x==this.defaultValue)w=j}else if(j.style.display!="none")j.style.display="none";if(j.className!="")j.className=""}}if(a==null)m();else{if(!e()){i(h);r.showPopup(h);b=null}if(!b||b.style.display=="none"){b=w||a;b.parentNode.scrollTop=0;o=b.id}b.className="active";J(b)}}};this.editKeyUp=function(a,b){if(f!=null)if(c(a))if(!(!e()&&(b.keyCode==13||b.keyCode==9)))if(b.keyCode==27||
b.keyCode==37||b.keyCode==39)m();else if(a.value!=E){f=a.id;r.refilter(a.value)}else(a=o?d.getElement(o):null)&&J(a)};g.onclick=l;g.onscroll=function(){if(D){clearTimeout(D);var a=d.getElement(f);a&&a.focus()}};this.delayHide=function(a){D=setTimeout(function(){D=null;if(g&&(a==null||f==a.id))m()},300)}});
