    ...
    function Utils() {
      ...
      this.transform_inverted = function(t) {
        ...
        if (det != 0) {
          ...
          return [m11 / det, m12 / det, m21 / det, m22 / det, m31 / det, m32 / det];
        } else {
      ...
      this.drawStencilAlongPath = function(ctx, stencil, path, fill, stroke, softClipping) {
        ...
        for (i = 0; i < path.length; i++) {
          ...
          if (
            softClipping &&
            ctx.wtClipPath &&
            !self.pnpoly(s, self.transform_apply(ctx.wtClipPathTransform, ctx.wtClipPath))
          ) {
            continue;
          }
          if (
      ...
      this.drawText = function(ctx, rect, align, text, clipPoint) {
        if (
          clipPoint &&
          ctx.wtClipPath &&
          !self.pnpoly(clipPoint, self.transform_apply(ctx.wtClipPathTransform, ctx.wtClipPath))
        ) {
          return;
        }
            ...
            if (angle == 0) {
              for (j = 0; j < split.length; j++) {
                ...
                self.drawText(
                  ...
                  align,
                  split[j],
                  softClipping ? [x(tseg), y(tseg)] : null
                );
              }
            } else {
      ...
      this.drawImage = function(ctx, image, uri, src_rect, tgt_rect) {
        try {
          ctx.drawImage(
            image,
            src_rect[0],
