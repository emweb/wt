WT_DECLARE_WT_MEMBER(1,JavaScriptConstructor,"WSpinBox",function(A,c,t,g,f,k,l,m,p,q){function n(){return c.readOnly}function r(){var a=e.data("lobj"),b="";if(a!==undefined){b=a.getValue();if(b==="")b=g+"0"+f}else b=c.value;if(b.substr(0,g.length)==g){b=b.substr(g.length);if(b.length>f.length&&b.substr(b.length-f.length,f.length)==f){b=b.substr(0,b.length-f.length);if(q)b=b.split(q).join("");b=b.replace(p,".");return Number(b)}}return null}function u(a){var b=e.data("lobj");if(a>l)a=l;else if(a<k)a=
k;a=a.toFixed(t);a=a.replace(".",p);var i=a.indexOf(p),h="";if(i!==-1){for(var j=0;j<i;j++){h+=a.charAt(j);if(j<i-1&&(i-j-1)%3===0)h+=q}h+=a.substr(i)}else h=a;if(b!==undefined)b.setValue(g+h+f);else c.value=g+h+f;s=true}function w(){var a=r();if(a!==null){a+=m;u(a)}}function x(){var a=r();if(a!==null){a-=m;u(a)}}jQuery.data(c,"obj",this);var d=A.WT,e=$(c),o=null,y,s=false,z=null,v=false;this.setIsDoubleSpinBox=function(a){v=a;this.configure(t,g,f,k,l,m)};this.configure=function(a,b,i,h,j,B){t=a;
