...
b,d)}else if(c.style.height!=d+"px"){c.style.height=d+"px";if(c.className=="Wt-wrapdiv")if(a.isIE&&a.hasTag(c.firstChild,"TEXTAREA"))c.firstChild.style.height=d-a.pxself(c,"marginBottom")+"px"}}};this.adjustRow=function(b,d){var g=[];if(b.style.height!=d+"px")b.style.height=d+"px";b=b.childNodes;var j,c,e,h;j=0;h=-1;for(c=b.length;j<c;++j){e=b[j];e.className!="Wt-vrh"&&++h;if(e.rowSpan!=1){this.adjustCell(e,0,h);g.push({td:e,col:h})}else this.adjustCell(e,d,h)}return g};this.adjust=function(){var b=
a.getElement(t);if(!b)return false;o.initResize&&o.initResize(a,t,i);if(a.isHidden(b))return true;var d=b.firstChild,g=b.parentNode;if(d.style.height!=="")d.style.height="";var j=a.pxself(g,"height"),c=false;if(j===0){c=true;j=g.clientHeight}var e=g.clientWidth;if(!(b.dirty||d.w!==e||d.h!==j))return true;d.w=e;d.h=j;b.dirty=null;var h=j;if(c){h-=a.px(g,"paddingTop");h-=a.px(g,"paddingBottom")}h-=a.px(b,"marginTop");h-=a.px(b,"marginBottom");if(g.children&&g.children.length!==1){c=0;for(j=g.children.length;c<
...
0;for(j=b.length;c<j;++c){e=b[c].td;h=b[c].col;this.adjustCell(e,e.offsetHeight,h)}if(d.style.tableLayout!=="fixed")return true;p=0;r=d.childNodes;var y,A,z;b=0;for(g=r.length;b<g;b++){h=r[b];if(a.hasTag(h,"COLGROUP")){b=-1;r=h.childNodes;g=r.length}if(a.hasTag(h,"COL")){if(a.pctself(h,"width")===0){c=s=0;for(j=d.rows.length;c<j;c++){e=d.rows[c];n=e.childNodes;A=y=0;for(z=n.length;A<z;A++){e=n[A];if(e.colSpan===1&&y===p&&e.childNodes.length===1){e=e.firstChild;e=e.offsetWidth+o.marginH(e);s=Math.max(s,
e);break}y+=e.colSpan;if(y>p)break}}if(s>0&&a.pxself(h,"width")!==s)h.style.width=s+"px"}++p}}return true};this.contains=function(b){var d=a.getElement(t);b=a.getElement(b.getId());return d&&b?a.contains(d,b):false};this.adjust()});
