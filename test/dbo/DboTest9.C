...
BOOST_AUTO_TEST_CASE( dbo9_query_using_single_with )
{
  Dbo9Fixture f;
  dbo::Session &session = *f.session_;

  {
    ...
    auto q = session.query<dbo::ptr<Painting>>(
      "select p from \"painting\" as p "
      "where p.price > (select MAX(ap.price) from artist_paintings as ap)")
      .with("artist_paintings", "select * from \"painting\" where artist = ?")
      .bind("Vermeer");
...
BOOST_AUTO_TEST_CASE( dbo9_query_using_single_multiple_with )
{
  Dbo9Fixture f;
  dbo::Session &session = *f.session_;

  {
    ...
    auto q = session.query<dbo::ptr<Painting>>(
      "select p from \"painting\" as p "
      "where p.price > (select mp.price from max_price as mp)")
      .with("artist_paintings", "select * from \"painting\" where artist = ?")
      .with("max_price (price)", "select MAX(ap.price) from artist_paintings as ap")
      .bind("Vermeer");
