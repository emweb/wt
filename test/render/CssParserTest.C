...
BOOST_AUTO_TEST_CASE( CssParser_test1 )
{
  ...
  BOOST_REQUIRE( !parser.parse("h1 h2 h3 & h4 {inside: ok}") );
  BOOST_REQUIRE(  parser.parse("h1{}") );
  BOOST_REQUIRE( !parser.parse("1h{}") );
  BOOST_REQUIRE( !parser.parse("h 1{}") );
  BOOST_REQUIRE(  parser.parse(".class1{}") );
  BOOST_REQUIRE( !parser.parse(".1class{}") );
  BOOST_REQUIRE(  parser.parse("#id1{}") );
  BOOST_REQUIRE( !parser.parse("#1id{}") );
  BOOST_REQUIRE( !parser.parse("{}") );
  BOOST_REQUIRE(  parser.parse("id_{id_:boo}") );
  Wt::Render::StyleSheet* s6 = parser.parse("a{inside:\"}b{\"}");
  BOOST_REQUIRE(  s6 );
  BOOST_REQUIRE(  s6->rulesetSize() == 1 );
  BOOST_REQUIRE(  parser.parse("h1{ a: a; b: b }") );
  BOOST_REQUIRE(  parser.parse("h1{ a: a; b: b; }") );
  BOOST_REQUIRE(  parser.parse("h1{ a: 2em }") );
  ...
  // Test hex color
  BOOST_REQUIRE(  parser.parse("h1{color:#123}") );
  BOOST_REQUIRE(  parser.parse("h1{color:#a11}") );
  BOOST_REQUIRE( !parser.parse("h1{color:#11}") );
  BOOST_REQUIRE(  parser.parse("h1{color:#123456}") );
  BOOST_REQUIRE( !parser.parse("h1{color:#ggg}") );

  // Test multi term expressions
  BOOST_REQUIRE( parser.parse("h1{test: .1px}") );
  BOOST_REQUIRE( parser.parse("hr{border: 1px 1px}") );
  BOOST_REQUIRE( parser.parse("hr{outline: thin dotted invert}") );

  // Test comments
  BOOST_REQUIRE( parser.parse("/*bla*/ h1{}") );
  BOOST_REQUIRE( parser.parse("h1{/*bla*/ test: 1px}") );
  BOOST_REQUIRE( parser.parse("h1{test: /*bla*/1px}") );
  BOOST_REQUIRE( parser.parse("h1{test: /*bla* */1px}") );
  BOOST_REQUIRE( parser.parse("<!--bla--> h1{test: 1px}") );
  BOOST_REQUIRE( parser.parse("h1{test: 1px <!--bla--> }") );
  BOOST_REQUIRE( parser.parse("h1{test: 1px <!--b-l-a--> }") );

  // Test white spaces
  BOOST_REQUIRE( parser.parse("h1{test:\t\n\r\f 1px}") );

  // Test URI's
  BOOST_REQUIRE( parser.parse("h1{test:url(\'bla\')}") );
  BOOST_REQUIRE( parser.parse("h1{test:url(\"bla\")}") );
  BOOST_REQUIRE( parser.parse("h1{test:url(bla)}") );
  BOOST_REQUIRE( parser.parse("h1{test:url( \"bla\" )}") );
  BOOST_REQUIRE( parser.parse("h1{test:url( \t\n\f\r \"bla\" )}") );
  BOOST_REQUIRE( parser.parse("h1{test:url(\"folder\\image.gif\" )}") );
...
{
  Wt::Render::CssParser parser;
  BOOST_REQUIRE( !parser.parseFile("") );
  BOOST_REQUIRE(  parser.parseFile("../resources/html4_default.css") );
}
