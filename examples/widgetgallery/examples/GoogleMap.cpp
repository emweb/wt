...
public:
      ...
      : Wt::WContainerWidget(parent)
  {
    ...
    Wt::WPushButton *brussels = new Wt::WPushButton("Brussels");
    controls->bindWidget("brussels", brussels);

    brussels->clicked().connect(std::bind([=] () {
            this->panToBrussels();
    }));

    Wt::WPushButton *lisbon = new Wt::WPushButton("Lisbon");
    controls->bindWidget("lisbon", lisbon);

    lisbon->clicked().connect(std::bind([=] () {
            this->panToLisbon();
    }));

    Wt::WPushButton *paris = new Wt::WPushButton("Paris");
    controls->bindWidget("paris", paris);

    paris->clicked().connect(std::bind([=] () {
            this->panToParis();
    }));
    ...
    if (map_->apiVersion() == Wt::WGoogleMap::Version2)
      addMapTypeControl("Hierarchical", Wt::WGoogleMap::HierarchicalControl);
    if (map_->apiVersion() == Wt::WGoogleMap::Version3)
      addMapTypeControl("Horizontal bar",Wt::WGoogleMap::HorizontalBarControl);
    ...
    map_->doubleClicked().connect(std::bind(
...
private:
  void panToBrussels() {
        map_->panTo(Wt::WGoogleMap::Coordinate(50.85034,4.35171));
  }

  void panToLisbon() {
        map_->panTo(Wt::WGoogleMap::Coordinate(38.703731,-9.135475));
  }

  void panToParis() {
        map_->panTo(Wt::WGoogleMap::Coordinate(48.877474, 2.312579));
  }
