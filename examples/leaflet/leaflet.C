...
LeafletApplication::LeafletApplication(const Wt::WEnvironment &env)
  : WApplication(env),
    map_(new Wt::WLeafletMap(root())),
    marker_(new Wt::WLeafletMap::LeafletMarker(Wt::WLeafletMap::Coordinate(50.906705, 4.655800))),
    markerAdded_(false)
{
  map_->resize(500, 500);
  ...
  Wt::WPushButton *panToEmwebBtn = new Wt::WPushButton(Wt::utf8("pan to Emweb"), root());
  panToEmwebBtn->clicked().connect(this, &LeafletApplication::panToEmweb);

  Wt::WPushButton *panToParisBtn = new Wt::WPushButton(Wt::utf8("pan to Paris"), root());
